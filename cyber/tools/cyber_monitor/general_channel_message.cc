...
GeneralChannelMessage* GeneralChannelMessage::OpenChannel(const std::string& channelName) 
{
        ...
        auto callBack = [this](const std::shared_ptr<apollo::cyber::message::RawMessage>& rawMsg) 
        {
                updateRawMessage(rawMsg);
        };

        channel_reader_ = channel_node_->CreateReader<apollo::cyber::message::RawMessage>(channelName, callBack);
...
void GeneralChannelMessage::RenderDebugString(const Screen* s, int key, unsigned lineNo) 
{
        if (has_message_come()) 
        {
                ...
                else 
                {
                        ...
                        if (channelMsg->message.size()) 
                        {
                                ...
                                {
                                        outStr << " (" << static_cast<float>(channelMsg->message.size()) / kGB << " GB)";
                                } 
                                else if (channelMsg->message.size() >= kMB) 
                                {
                                        outStr << " (" << static_cast<float>(channelMsg->message.size()) / kMB << " MB)";
                                } 
