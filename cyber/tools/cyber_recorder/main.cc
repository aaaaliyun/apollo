...
int main(int argc, char** argv) 
{
  ...
  auto opt_header = HeaderBuilder::GetHeader();
        ...
        else if (command == "record") 
        {
                ...
                {
                        std::cout << "MUST specify channels option (-c) or all channels option (-a)."
                                  << std::endl;
                        return -1;
                }
                ...
                ::apollo::cyber::Init(argv[0]);
                auto recorder = std::make_shared<Recorder>(opt_output_vec[0], opt_all, opt_white_channels, opt_header);
