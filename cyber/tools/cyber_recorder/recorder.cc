...
void Recorder::FindNewChannel(const RoleAttributes& role_attr) 
{
        ...
        if (!all_channels_ && std::find(channel_vec_.begin(), channel_vec_.end(), role_attr.channel_name()) == channel_vec_.end()) 
        {
...
bool Recorder::InitReadersImpl() 
{
        ...
        change_conn_ = channel_manager->AddChangeListener(std::bind(&Recorder::TopologyCallback, this, std::placeholders::_1));
