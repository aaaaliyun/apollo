...
private:
        // NOTE: lock hold
        template <typename MessageT>
        std::pair<std::shared_ptr<ListenerHandler<MessageT>>, bool> GetHandler(uint64_t channel_id, const std::string& message_type, BaseHandlersType* handlers) 
        {
        ...
        template <typename MessageT>
        void Run(const uint64_t channel_id, const std::string& message_type, const BaseHandlersType& handlers, const std::shared_ptr<MessageT>& message, const MessageInfo& message_info) 
        {
...
template <typename MessageT>
void IntraDispatcher::OnMessage(uint64_t channel_id, const std::shared_ptr<MessageT>& message, const MessageInfo& message_info) 
{
...
void IntraDispatcher::AddListener(const RoleAttributes& self_attr, const MessageListener<MessageT>& listener) 
{
        ...
        if (handler && created) 
        {
                auto listener_wrapper = [this, self_id, channel_id, message_type](const std::shared_ptr<MessageT>& message, const MessageInfo& message_info) 
                {
                                  ...
                                  const MessageListener<MessageT>& listener) 
{
        ...
        if (handler && created) 
        {
                auto listener_wrapper = [this, self_id, oppo_id, channel_id, message_type](const std::shared_ptr<MessageT>& message, const MessageInfo& message_info) 
                {
...
void IntraDispatcher::RemoveListener(const RoleAttributes& self_attr, const RoleAttributes& opposite_attr) 
{
        ...
        chain_->RemoveListener<MessageT>(self_attr.id(), opposite_attr.id(), self_attr.channel_id(), message::GetMessageName<MessageT>());
}
