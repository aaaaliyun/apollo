...
ParameterClient::ParameterClient(const std::shared_ptr<Node>& node, const std::string& service_node_name) : node_(node) 
{
        get_parameter_client_ = node_->CreateClient<ParamName, Param>(FixParameterServiceName(service_node_name, GET_PARAMETER_SERVICE_NAME));

        set_parameter_client_ = node_->CreateClient<Param, BoolResult>(FixParameterServiceName(service_node_name, SET_PARAMETER_SERVICE_NAME));

        list_parameters_client_ = node_->CreateClient<NodeName, Params>(FixParameterServiceName(service_node_name, LIST_PARAMETERS_SERVICE_NAME));
}
