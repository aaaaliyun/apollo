...
public:
        PyService(const std::string& service_name, const std::string& data_type, Node* node)
                : node_(node), service_name_(service_name), data_type_(data_type), func_(nullptr) 
        {
                auto f = [this](const std::shared_ptr<const message::PyMessageWrap>& request, std::shared_ptr<message::PyMessageWrap>& response) 
                {
                        response = this->cb(request);
                };
                service_ = node_->CreateService<message::PyMessageWrap, message::PyMessageWrap>(service_name, f);
        }
...
{
public:
        PyClient(const std::string& name, const std::string& data_type, Node* node) : node_(node), service_name_(name), data_type_(data_type) 
        {
...
public:
        static std::vector<std::string> get_active_nodes(uint8_t sleep_s = 2) 
        {
                ...
                std::sort(nodes.begin(), nodes.end(), [](const RoleAttributes& na, const RoleAttributes& nb) -> bool 
                {
...
public:
        static std::vector<std::string> get_active_services(uint8_t sleep_s = 2) 
        {
                ...
                std::sort(services.begin(), services.end(), [](const RoleAttributes& sa, const RoleAttributes& sb) -> bool 
                {
