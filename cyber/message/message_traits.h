...
template <typename T, typename std::enable_if<HasType<T>::value &&
...
template <typename T>
typename std::enable_if<HasSetType<T>::value, void>::type 
SetTypeName(const std::string& type_name, T* message) 
{
...
ParseFromHC(const void* data, int size, T* message) 
{
        ...
        return message->ParseFromArray(static_cast<const void*>(static_cast<const char*>(data) + header_size), header->content_size());
}
