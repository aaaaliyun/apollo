...
double CalculateAcceleration(const Point3D &acceleration, const Point3D &velocity, const apollo::canbus::Chassis_GearPosition &gear_location) 
{
...
SimulationWorldService::SimulationWorldService(const MapService *map_service, bool routing_from_file)
    : node_(cyber::CreateNode("simulation_world")), map_service_(map_service), monitor_logger_buffer_(MonitorMessageItem::SIMULATOR), ready_to_push_(false) 
{
        InitReaders();
        InitWriters();
...
void SimulationWorldService::InitReaders() 
{
        ...
        perception_obstacle_reader_ = node_->CreateReader<PerceptionObstacles>(FLAGS_perception_obstacle_topic);
        perception_traffic_light_reader_ = node_->CreateReader<TrafficLightDetection>(FLAGS_traffic_light_detection_topic);
        ...
        drive_event_reader_ = node_->CreateReader<DriveEvent>(FLAGS_drive_event_topic, [this](const std::shared_ptr<DriveEvent> &drive_event) 
        {
        ...
        monitor_reader_ = node_->CreateReader<MonitorMessage>(monitor_message_reader_config, [this](const std::shared_ptr<MonitorMessage> &monitor_message) 
        {
...
void SimulationWorldService::GetWireFormatString(double radius, std::string *sim_world, std::string *sim_world_with_planning_data) 
{
...
void SimulationWorldService::UpdateSimulationWorld(const LocalizationEstimate &localization) 
{
        ...
        auto_driving_car->set_speed_acceleration(CalculateAcceleration(pose.linear_acceleration(), pose.linear_velocity(), gear_location_));
...
void SimulationWorldService::UpdateSimulationWorld(const Gps &gps) 
{
        ...
        else 
        {
                ...
                double heading = apollo::common::math::QuaternionToHeading(pose.orientation().qw(), pose.orientation().qx(),
...
void SimulationWorldService::UpdateSimulationWorld(const Chassis &chassis) 
{
        ...
        double kappa = std::tan(steering_angle / vehicle_param.steer_ratio()) / vehicle_param.wheel_base();
...
void SimulationWorldService::CreateWorldObjectFromSensorMeasurement(const SensorMeasurement &sensor, Object *world_object) 
{
...
void SimulationWorldService::SetObstacleInfo(const PerceptionObstacle &obstacle, Object *world_object) 
{
        if (world_object == nullptr) 
        {
                return;
...
void SimulationWorldService::SetObstaclePolygon(const PerceptionObstacle &obstacle, Object *world_object) 
{
        if (world_object == nullptr) 
        {
                return;
...
void SimulationWorldService::SetObstacleSensorMeasurements(const PerceptionObstacle &obstacle, Object *world_object) 
{
        if (world_object == nullptr) 
        {
                return;
        ...
        {
                Object *obj = (*(world_.mutable_sensor_measurements()))[sensor.sensor_id()].add_sensor_measurement();
...
template <>
void SimulationWorldService::UpdateSimulationWorld(const TrafficLightDetection &traffic_light_detection) 
{
...
void SimulationWorldService::UpdateRSSInfo(const ADCTrajectory &trajectory) 
{
        if (trajectory.has_rss_info()) 
        {
                ...
                else 
                {
                        ...
                        this->PublishMonitorMessage(MonitorMessageItem::ERROR,
                                                   "RSS unsafe: \ncurrent distance: " + formatDoubleToString(trajectory.rss_info().cur_dist_lon()) + "\nsafe distance: " + formatDoubleToString(trajectory.rss_info().rss_safe_dist_lon()));
...
void SimulationWorldService::UpdateMainStopDecision(const apollo::planning::MainDecision &main_decision, double update_timestamp_sec, Object *world_main_decision) 
{
...
bool SimulationWorldService::LocateMarker(const apollo::planning::ObjectDecisionType &decision, Decision *world_decision) 
{
...
void SimulationWorldService::FindNudgeRegion(const apollo::planning::ObjectDecisionType &decision, const Object &world_obj, Decision *world_decision) 
{
        ...
        const apollo::common::math::Polygon2d &nudge_polygon = obj_polygon.ExpandByDistance(std::fabs(decision.nudge().distance_l()));
...
void SimulationWorldService::UpdateDecision(const DecisionResult &decision_res, double header_time) 
{
        // Update turn signal.
        ...
        for (const auto &obj_decision : decision_res.object_decision().decision()) 
        {
                if (obj_decision.has_perception_id()) 
                {
                        ...
                        for (const auto &decision : obj_decision.object_decision()) 
                        {
                                ...
                                if (decision.has_stop() || decision.has_follow() || decision.has_yield() || decision.has_overtake()) 
                                {
...
void SimulationWorldService::UpdatePlanningData(const PlanningData &data) 
{
        ...
        for (auto &graph : data.st_graph()) 
        {
                ...
                DownsampleSpeedPointsByInterval(graph.speed_profile(), profile_downsample_interval, st_graph->mutable_speed_profile());
                ...
                DownsampleSpeedPointsByInterval(graph.speed_limit(), limit_downsample_interval, st_graph->mutable_speed_limit());
        }
        ...
        for (auto &plan : data.speed_plan()) 
        {
                if (plan.speed_point_size() > 0) 
                {
                        ...
                        DownsampleSpeedPointsByInterval(plan.speed_point(), interval, downsampled_plan->mutable_speed_point());
                }
        }
...
{
        {
                ...
                if (world_.has_routing_time() && world_.routing_time() == routing_response.header().timestamp_sec()) 
                {
...
{
        const int updated_size = std::min(monitor_msg.item_size(), SimulationWorldService::kMaxMonitorItems);
...
void SimulationWorldService::PublishNavigationInfo(const std::shared_ptr<NavigationInfo> &navigation_info) 
{
        FillHeader(FLAGS_dreamview_module_name, navigation_info.get());
        navigation_writer_->Write(navigation_info);
}

void SimulationWorldService::PublishRoutingRequest(const std::shared_ptr<RoutingRequest> &routing_request) 
{
        FillHeader(FLAGS_dreamview_module_name, routing_request.get());
        routing_request_writer_->Write(routing_request);
}

void SimulationWorldService::PublishMonitorMessage(apollo::common::monitor::MonitorMessageItem::LogLevel log_level, const std::string &msg) 
{
