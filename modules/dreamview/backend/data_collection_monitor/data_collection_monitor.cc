...
void DataCollectionMonitor::LoadConfiguration() 
{
        ...
        {
                AWARN << "No corresponding data collection table file found in " << vehicle_dir << ". Using default one instead.";
                data_collection_config_path = FLAGS_default_data_collection_config_path;
        }

        ACHECK(cyber::common::GetProtoFromFile(data_collection_config_path, &data_collection_table_)) << "Unable to parse data collection configuration from file " << data_collection_config_path;
    ...
    std::string current_category_name, const Category& current_category) 
{
        ...
        for (const auto& range : feature.range()) 
        {
                ...
                ConstructCategoriesHelper(scenario_name, scenario, feature_idx + 1, new_category_name, new_category);
        }
}
...
void DataCollectionMonitor::OnChassis(const std::shared_ptr<Chassis>& chassis) 
{
        ...
        for (const auto& scenario_iter : scenario_to_categories_) 
        {
                ...
                for (const auto& category_iter : categories) 
                {
                        ...
                        const size_t consecutive_count = ++category_consecutive_frame_count_[scenario_name][category_name];
                        ...
                        const double progress_percentage = 100.0 * static_cast<double>(category_frame_count_[scenario_name][category_name]) / static_cast<double>(total_frames);
                        {
    ...
    const std::shared_ptr<Chassis>& chassis, const Category& category) 
{
        ...
        for (const auto& range : category) 
        {
                for (const auto& criterion : range.criterion()) 
                {
                        ...
                        else if (!GetProtobufFloatByFieldName(vehicle_param, vehicle_param_descriptor, vehicle_param_reflection, criterion.vehicle_config(), &target_value)) 
                        {
                                return false;
                        }

                        float actual_value;
                        if (!GetProtobufFloatByFieldName(*chassis, chassis_descriptor, chassis_reflection, criterion.field(), &actual_value)) 
                        {
                                return false;
                        }
