...
const std::string remote_daemon_cmd_channel = "/apollo/teleop/remote/daemon_service/cmd";
const std::string remote_daemon_rpt_channel = "/apollo/teleop/remote/daemon_service/rpt";
const std::string local_daemon_cmd_channel = "/apollo/teleop/local/daemon_service/cmd";
const std::string local_daemon_rpt_channel = "/apollo/teleop/local/daemon_service/rpt";
...
void TeleopService::Start() 
{
        ...
        modem0_info_reader_ = node_->CreateReader<ModemInfo>(modem0_channel, [this](const std::shared_ptr<ModemInfo> &msg) 
        {
                UpdateModem(modem0_id, msg);
        });

        modem1_info_reader_ = node_->CreateReader<ModemInfo>(modem1_channel, [this](const std::shared_ptr<ModemInfo> &msg) 
        {
                UpdateModem(modem1_id, msg);
        });

        modem2_info_reader_ = node_->CreateReader<ModemInfo>(modem2_channel, [this](const std::shared_ptr<ModemInfo> &msg) 
        {
                UpdateModem(modem2_id, msg);
        });

        planning_reader_ = node_->CreateReader<ADCTrajectory>(planning_channel, [this](const std::shared_ptr<ADCTrajectory> &msg) 
        {
                UpdatePlanning(msg);
        });

        remote_daemon_cmd_writer_ = node_->CreateWriter<DaemonServiceCmd>(remote_daemon_cmd_channel);

        local_daemon_cmd_writer_ = node_->CreateWriter<DaemonServiceCmd>(local_daemon_cmd_channel);

        remote_daemon_rpt_reader_ = node_->CreateReader<DaemonServiceRpt>(remote_daemon_rpt_channel, [this](const std::shared_ptr<DaemonServiceRpt> &msg) 
        {
                UpdateCarDaemonRpt(msg);
        });

        local_daemon_rpt_reader_ = node_->CreateReader<DaemonServiceRpt>(local_daemon_rpt_channel, [this](const std::shared_ptr<DaemonServiceRpt> &msg) 
        {
...
void TeleopService::RegisterMessageHandlers() 
{
        ...
        websocket_->RegisterConnectionReadyHandler([this](WebSocketHandler::Connection *conn) { SendStatus(conn); });
        // Start/Stop local and remote audio
        websocket_->RegisterMessageHandler("ToggleAudio", [this](const Json &json, WebSocketHandler::Connection *conn) 
        {
                {
        ...
        websocket_->RegisterMessageHandler("ToggleMic", [this](const Json &json, WebSocketHandler::Connection *conn) 
        {
                bool start = false;
                {
                        boost::unique_lock<boost::shared_mutex> writer_lock(mutex_);
        ...
        // compositor
        websocket_->RegisterMessageHandler("ToggleVideo", [this](const Json &json, WebSocketHandler::Connection *conn) 
        {
                bool start = false;  // false means stop
                // create a scope for the mutex lock
                {
        ...
        websocket_->RegisterMessageHandler("PullOver", [this](const Json &json, WebSocketHandler::Connection *conn) 
        {
                boost::shared_lock<boost::shared_mutex> reader_lock(mutex_);
                SendPullOverCmd();
        });
        // Issue emergency-stop command to remote
        websocket_->RegisterMessageHandler("EStop", [this](const Json &json, WebSocketHandler::Connection *conn) 
        {
                boost::shared_lock<boost::shared_mutex> reader_lock(mutex_);
                SendEstopCmd();
        });
        // Issue resume-cruise command to remote
        websocket_->RegisterMessageHandler("ResumeCruise", [this](const Json &json, WebSocketHandler::Connection *conn) 
        {
                boost::shared_lock<boost::shared_mutex> reader_lock(mutex_);
                SendResumeCruiseCmd();
        });
        // Request to get updated modem info for client display
        websocket_->RegisterMessageHandler("RequestTeleopStatus", [this](const Json &json, WebSocketHandler::Connection *conn) 
        {
                SendStatus(conn);
        });
}
...
void TeleopService::UpdateCarDaemonRpt(const std::shared_ptr<DaemonServiceRpt> &daemon_rpt) 
{
        {
...
void TeleopService::UpdateOperatorDaemonRpt(const std::shared_ptr<DaemonServiceRpt> &daemon_rpt) 
{
        {
                bool voipIsRunning = false;
                for (int i = 0; i < daemon_rpt->services_size(); i++) 
