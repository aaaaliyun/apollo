...
void PointCloudUpdater::RegisterMessageHandlers() 
{
        ...
        websocket_->RegisterMessageHandler("RequestPointCloud", [this](const Json &json, WebSocketHandler::Connection *conn) 
        {
                std::string to_send;
                // If there is no point_cloud data for more than 2 seconds, reset.
                if (point_cloud_str_ != "" && std::fabs(last_localization_time_ - last_point_cloud_time_) > 2.0) 
                {
                        boost::unique_lock<boost::shared_mutex> writer_lock(mutex_);
                        point_cloud_str_ = "";
                }
        ...
        websocket_->RegisterMessageHandler("TogglePointCloud", [this](const Json &json, WebSocketHandler::Connection *conn) 
        {
...
{
        localization_reader_ = node_->CreateReader<LocalizationEstimate>(FLAGS_localization_topic, [this](const std::shared_ptr<LocalizationEstimate> &msg) 
        {
                UpdateLocalizationTime(msg);
        });

        point_cloud_reader_ = node_->CreateReader<drivers::PointCloud>(FLAGS_pointcloud_topic, [this](const std::shared_ptr<drivers::PointCloud> &msg) 
        {
...
pcl::PointCloud<pcl::PointXYZ>::Ptr PointCloudUpdater::ConvertPCLPointCloud(const std::shared_ptr<drivers::PointCloud> &point_cloud) 
{
        ...
        if (point_cloud->width() * point_cloud->height() != static_cast<unsigned int>(point_cloud->point_size())) 
        {
                pcl_ptr->width = 1;
...
void PointCloudUpdater::UpdatePointCloud(const std::shared_ptr<drivers::PointCloud> &point_cloud) 
{
        ...
        if (simworld_updater_->LastAdcTimestampSec() == 0.0 || simworld_updater_->LastAdcTimestampSec() - last_point_cloud_time_ > 0.1) 
        {
...
void PointCloudUpdater::FilterPointCloud(pcl::PointCloud<pcl::PointXYZ>::Ptr pcl_ptr) 
{
        ...
        */
        if (enable_voxel_filter_) 
        {
                ...
                voxel_grid.setLeafSize(static_cast<float>(FLAGS_voxel_filter_size), static_cast<float>(FLAGS_voxel_filter_size), static_cast<float>(FLAGS_voxel_filter_height));
...
void PointCloudUpdater::UpdateLocalizationTime(const std::shared_ptr<LocalizationEstimate> &localization) 
{
