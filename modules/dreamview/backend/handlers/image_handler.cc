...
ImageHandler::ImageHandler() : requests_(0), node_(cyber::CreateNode("image_handler")) 
{
        node_->CreateReader<Image>(FLAGS_image_front_topic, [this](const std::shared_ptr<Image> &image) 
        { 
                OnImageFront(image); 
        });

        node_->CreateReader<CompressedImage>(FLAGS_image_short_topic, [this](const std::shared_ptr<CompressedImage> &image) 
        {
                OnImageShort(image);
        });
}
