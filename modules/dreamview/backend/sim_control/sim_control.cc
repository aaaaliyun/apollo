...
void SimControl::InitTimerAndIO() 
{
        ...
        planning_reader_ = node_->CreateReader<ADCTrajectory>(FLAGS_planning_trajectory_topic, [this](const std::shared_ptr<ADCTrajectory>& trajectory) 
        {
                this->OnPlanning(trajectory);
        });
        routing_response_reader_ = node_->CreateReader<RoutingResponse>(FLAGS_routing_response_topic, [this](const std::shared_ptr<RoutingResponse>& routing) 
        {
                this->OnRoutingResponse(routing);
        });
        navigation_reader_ = node_->CreateReader<NavigationInfo>(FLAGS_navigation_topic, [this](const std::shared_ptr<NavigationInfo>& navigation_info) 
        {
                this->OnReceiveNavigationInfo(navigation_info);
        });
        prediction_reader_ = node_->CreateReader<PredictionObstacles>(FLAGS_prediction_topic, [this](const std::shared_ptr<PredictionObstacles>& obstacles) 
        {
        ...
        sim_control_timer_.reset(new cyber::Timer(kSimControlIntervalMs, [this]() { this->RunOnce(); }, false));
        sim_prediction_timer_.reset(new cyber::Timer(kSimPredictionIntervalMs, [this]() { this->PublishDummyPrediction(); }, false));
}
...
void SimControl::InitStartPoint(double start_velocity, double start_acceleration) 
{
        ...
        else 
        {
                ...
                // deceleration.
                double projection = pose.linear_acceleration().x() * pose.linear_velocity().x() + pose.linear_acceleration().y() * pose.linear_velocity().y();
...
void SimControl::OnRoutingResponse(const std::shared_ptr<RoutingResponse>& routing) 
{
        ...
        CHECK_GE(routing->routing_request().waypoint_size(), 2) << "routing should have at least two waypoints";
...
void SimControl::Start() 
{
        ...
        if (!enabled_) 
        {
                ...
                localization_reader_->Observe();
                Init(localization_reader_->Empty(), next_point_.has_v() ? next_point_.v() : 0.0, next_point_.has_a() ? next_point_.a() : 0.0);
...
bool SimControl::PerfectControlModel(TrajectoryPoint* point, Chassis::GearPosition* gear_position) 
{
        ...
        *gear_position = current_trajectory_->gear();
        ...
        else 
        {
                ...
                else 
                {
                        ...
                        while (next_point_index_ < trajectory.size() &&
                               current_time > trajectory.Get(next_point_index_).relative_time() + current_trajectory_->header().timestamp_sec()) 
                        {
                                ++next_point_index_;
                        }
        ...
        {
                *point = InterpolateUsingLinearApproximation(prev_point_, next_point_, current_time - current_trajectory_->header().timestamp_sec());
        }
        return true;
}
...
void SimControl::PublishLocalization(const TrajectoryPoint& point) 
{
        ...
        TransformToVRF(pose->angular_velocity(), pose->orientation(), pose->mutable_angular_velocity_vrf());
        ...
        TransformToVRF(pose->linear_acceleration(), pose->orientation(), pose->mutable_linear_acceleration_vrf());
