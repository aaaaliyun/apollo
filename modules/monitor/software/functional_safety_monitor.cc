...
bool IsSafe(const std::string& name, const ComponentStatus& status) 
{
        ...
        {
                MonitorManager::Instance()->LogBuffer().ERROR(absl::StrCat(name, " triggers safe mode: ", status.message()));
                return false;
        }
        return true;
}
...
void FunctionalSafetyMonitor::RunOnce(const double current_time) 
{
        ...
        if (system_status->safety_mode_trigger_time() + FLAGS_safety_mode_seconds_before_estop < current_time) 
        {
...
bool FunctionalSafetyMonitor::CheckSafety() 
{
        ...
        for (const auto& iter : mode.monitored_components()) 
        {
                ...
                if (component.required_for_safety() && !IsSafe(component_name, components.at(component_name).summary())) 
                {
                        return false;
                }
        }
