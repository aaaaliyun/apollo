...
public:
        bool Init() 
        {
                localization_reader_ = node_->CreateReader<LocalizationEstimate>(FLAGS_localization_topic, [this](const std::shared_ptr<LocalizationEstimate> &localization) 
                {
                        ADEBUG << "Received chassis data: run chassis callback.";
                        OnLocalization(localization);
                });

                traffic_light_detection_writer_ = node_->CreateWriter<TrafficLightDetection>(FLAGS_traffic_light_detection_topic);
...
private:
        ...
        bool GetTrafficLightsWithinDistance(std::vector<SignalInfoConstPtr> *traffic_lights) 
        {
                ...
                int ret = hdmap->GetForwardNearestSignalsOnLane(position, FLAGS_traffic_light_distance, traffic_lights);
                if (ret != 0) 
                {
        ...
        bool CreateTrafficLightDetection(const std::vector<SignalInfoConstPtr> &signals, TrafficLight::Color color, TrafficLightDetection *detection) 
        {
...
private:
        ...
        std::shared_ptr<apollo::cyber::Writer<TrafficLightDetection>> traffic_light_detection_writer_ = nullptr;
