...
void FixedAspectRatioWidget::resizeEvent(QResizeEvent* revent) 
{
        ...
        {
                double aspect = static_cast<double>(viewer_.plane_.texWidth()) / static_cast<double>(viewer_.plane_.texHeight());
        ...
        viewer_.setGeometry((revent->size().width() - size.width()) / 2, (revent->size().height() - size.height()) / 2, size.width(), size.height());
