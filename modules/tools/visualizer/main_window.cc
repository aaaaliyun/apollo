      ...
      _channelName2TypeMap() 
{
        ...
        connect(pointcloud_comboBox_, SIGNAL(currentIndexChanged(int)), this, SLOT(ChangePointCloudChannel()));
...
void MainWindow::ActionAddGrid(void) 
{
        ...
        if (grid_root_item_ == nullptr) 
        {
                ...
                connect(ui_->treeWidget, SIGNAL(itemDoubleClicked(QTreeWidgetItem*, int)), this, SLOT(EditGridColor(QTreeWidgetItem*, int)));
...
_ret2:
        delete grid_;
_ret1:
        QMessageBox::warning(this, tr("Error"), tr("There is no enough memory for creating Grid!!!"), QMessageBox::Ok);
        return;
  }
}
...
{
        if (column && item != nullptr && grid_root_item_ != nullptr && item == grid_root_item_->child(0)) 
        {
...
void MainWindow::ActionOpenRadarChannel(void) 
{
        ...
        {
                radar_points_shader_ = RenderableObject::CreateShaderProgram(tr(radarVertexPath), tr(radarFragPath));
                if (radar_points_shader_ != nullptr) 
                {
                ...
                {
                        QMessageBox::warning(this, tr("NO Shader"), tr("There is no suitable shader for Radar Points!!!"), QMessageBox::Ok);
                        return;
                }
        }
        ...
        if (closed_radarData_list_.empty()) 
        {
                ...
                ui_->treeWidget->setItemWidget(&radarProxy->channel_name_item_, 1, &radarProxy->channel_name_combobox_);
        ...
        connect(&radarProxy->action_item_button_, SIGNAL(clicked(bool)), this, SLOT(openRadarChannel(bool)));
        connect(&radarProxy->enable_checkBox_, SIGNAL(clicked(bool)), this, SLOT(EnableRadarPoints(bool)));
        connect(&radarProxy->channel_name_combobox_, SIGNAL(currentIndexChanged(int)), this, SLOT(ChangeRadarChannel()));
...
void MainWindow::DoOpenRadarChannel(bool b, RadarData* radarProxy) 
{
        if (b) 
        {
                ...
                {
                        QMessageBox::warning(this, tr("Setup Channel Name"), tr("Channel Name cannot be empty!!!\nPlease select one!"), QMessageBox::Ok);
                ...
                if (!radarProxy->channel_reader_) 
                {
                        ...
                        if (!radarProxy->channel_reader_) 
                        {
                                QMessageBox::warning(this, tr("Create cyber Channel Reader"), tr("There is no enough memory!!!\nCannot create cyber channel reader!"), QMessageBox::Ok);
                                return;
                        }

                        auto radarcallback = [this, radarProxy](const std::shared_ptr<apollo::drivers::RadarObstacles>& pdata) 
                        {
                        ...
                        if (!radarProxy->channel_reader_->InstallCallbackAndOpen(radarcallback, radarProxy->channel_name_combobox_.currentText().toStdString(), nodeName)) 
                        {
                                QMessageBox::warning(this, tr("Setup Channel Callback"), tr("Channel Callback cannot be installed!!!\nPlease check it!"), QMessageBox::Ok);
...
void MainWindow::RadarRenderCallback(const std::shared_ptr<const apollo::drivers::RadarObstacles>& rawData, RadarData* radar) 
{
...
void MainWindow::ActionOpenPointCloud(void) 
{
        if (pointcloud_shader_ == nullptr) 
        {
                ...
                {
                        QMessageBox::warning(this, tr("NO Shader"), tr("There is no suitable shader for Pointcloud!!!"), QMessageBox::Ok);
                        return;
                }
        }
        ...
        if (pointcloud_top_item_ == nullptr) 
        {
                ...
                if (item == nullptr) 
                {
                        QMessageBox::warning(this, tr("NO Enough Memory"), tr("Cannot create tree item for channel name!!!"), QMessageBox::Ok);
                        return;
                }
                ...
                if (item == nullptr) 
                {
                        QMessageBox::warning(this, tr("NO Enough Memory"), tr("Cannot create tree item for channel Action!!!"), QMessageBox::Ok);
                        return;
                }
...
void MainWindow::ActionOpenImages(void) 
{
        if (open_images_dialog_ == nullptr) 
        {
                ...
                if (open_images_dialog_ == nullptr) 
                {
                        QMessageBox::warning(this, tr("No Enough Memory"), tr("There is no enough memory!!!"), QMessageBox::Ok);
                        return;
                } 
                else 
                {
...
void MainWindow::ActionOpenImage(void) 
{
        ...
        if (closed_video_image_viewer_list_.empty()) 
        {
                ...
                ui_->treeWidget->setItemWidget(&videoImgProxy->channel_name_item_, 1, &videoImgProxy->channel_name_combobox_);
                ui_->treeWidget->setItemWidget(&videoImgProxy->action_item_, 1, &videoImgProxy->action_item_button_);
        ...
        connect(&videoImgProxy->action_item_button_, SIGNAL(clicked(bool)), this, SLOT(PlayVideoImage(bool)));
        connect(&videoImgProxy->channel_name_combobox_, SIGNAL(currentIndexChanged(int)), this, SLOT(ChangeVideoImgChannel()));
        connect(&videoImgProxy->video_image_viewer_, SIGNAL(focusOnThis(FixedAspectRatioWidget*)), this, SLOT(SelectCurrentTreeItem(FixedAspectRatioWidget*)));
...
void MainWindow::DoDeleteVideoImg(VideoImgProxy* proxy) 
{
        ...
        disconnect(&proxy->channel_name_combobox_, SIGNAL(currentIndexChanged(int)), this, SLOT(ChangeVideoImgChannel()));
        disconnect(&proxy->video_image_viewer_, SIGNAL(focusOnThis(FixedAspectRatioWidget*)), this, SLOT(SelectCurrentTreeItem(FixedAspectRatioWidget*)));
...
void MainWindow::UpdateActions(void) 
{
        ...
        if (item) 
        {
                if (!item->parent() && item != pointcloud_top_item_ && item != all_channel_root_ && item != grid_root_item_) 
                {
...
void MainWindow::PointCloudReaderCallback(const std::shared_ptr<const apollo::drivers::PointCloud>& pdata) 
{
        pointcloud_reader_mutex_.lock();
        pointcloud_reader_mutex_.unlock();
...
void MainWindow::PlayRenderableObject(bool b) 
{
        if (b) 
        {
                ...
                {
                        QMessageBox::warning(this, tr("Setup Channel Name"), tr("Channel Name cannot be empty!!!\nPlease Select it!"), QMessageBox::Ok);
                ...
                if (!pointcloud_channel_Reader_) 
                {
                        ...
                        if (!pointcloud_channel_Reader_->InstallCallbackAndOpen(pointCallback, pointcloud_comboBox_->currentText().toStdString(), nodeName)) 
                        {
                                QMessageBox::warning(this, tr("Setup Channel Callback"), tr("Channel Callback cannot be installed!!!\nPlease check it!"), QMessageBox::Ok);
                                delete pointcloud_channel_Reader_;
                                pointcloud_channel_Reader_ = nullptr;
...
void MainWindow::ImageReaderCallback(const std::shared_ptr<const apollo::drivers::Image>& imgData, VideoImgProxy* theVideoImgProxy) 
{
...
void MainWindow::ImageReaderCallback(const std::shared_ptr<const apollo::drivers::CompressedImage>& imgData, VideoImgProxy* theVideoImgProxy) 
{
        theVideoImgProxy->reader_mutex_.lock();
        if (theVideoImgProxy->dynamic_texture_ != nullptr && imgData != nullptr) 
        {
                ...
                if (img.loadFromData(reinterpret_cast<const unsigned char*>(data.c_str()), static_cast<int>(data.size()))) 
                {
...
void MainWindow::DoPlayVideoImage(bool b, VideoImgProxy* theVideoImg) 
{
        if (b) 
        {
                ...
                if (!theVideoImg->image_reader_) 
                {
                        ...
                        {
                                theVideoImg->compressed_image_reader_ = new CyberChannReader<apollo::drivers::CompressedImage>();
                        } 
                        else 
                        {
                        ...
                        if (theVideoImg->isCompressedImage_) 
                        {
                                auto videoCallback = [this, theVideoImg](const std::shared_ptr<apollo::drivers::CompressedImage>& pdata) 
                                { 
                                        this->ImageReaderCallback(pdata, theVideoImg); 
                                };

                                ret = theVideoImg->compressed_image_reader_->InstallCallbackAndOpen(videoCallback, channelName, nodeName);
                        } 
                        else 
                        {
                                auto videoCallback = [this, theVideoImg](const std::shared_ptr<apollo::drivers::Image>& pdata) 
                                {
                                        this->ImageReaderCallback(pdata, theVideoImg);
                                };
                                ret = theVideoImg->image_reader_->InstallCallbackAndOpen(videoCallback, channelName, nodeName);
                        }
                        if (!ret) 
                        {
                                QMessageBox::warning(this, tr("Setup Channel Callback"), tr("Channel Callback cannot be installed!!!\nPlease check it!"), QMessageBox::Ok);
...
void MainWindow::ChangePointCloudChannel() 
{
        if (pointcloud_channel_Reader_ != nullptr) 
        {
                ...
                std::string nodeName("Visualizer-");
                nodeName.append(pointcloud_top_item_->text(0).toStdString());
                pointcloud_channel_Reader_->OpenChannel(pointcloud_comboBox_->currentText().toStdString(), nodeName);
        }
}
