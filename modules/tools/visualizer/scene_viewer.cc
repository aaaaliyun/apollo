...
void SceneViewer::AddNewShaderProg(const std::string &shaderProgName, const std::shared_ptr<QOpenGLShaderProgram> &newShaderProg) 
{
        ...
        QMatrix4x4 mvp = current_cameraPtr_->projection_matrix() * current_cameraPtr_->model_view_matrix();
...
void SceneViewer::mouseReleaseEvent(QMouseEvent *event) 
{
        if (!right_key_is_moved_ && event->button() == Qt::RightButton) 
        {
                if (camera_dialog_ == nullptr) 
                {
                        ...
                        if (!camera_dialog_) 
                        {
                                QMessageBox::warning(this, tr("Error"), tr("No Enough for creating Camera Dialog!!!"), QMessageBox::Ok);
                                goto retLabel;
                        } 
                        else 
                        {
                                ...
                                connect(camera_dialog_, SIGNAL(yawValueChanged(double)), this, SLOT(UpdateCameraYaw(double)));
                                connect(camera_dialog_, SIGNAL(pitchValueChanged(double)), this, SLOT(UpdateCameraPitch(double)));
                                connect(camera_dialog_, SIGNAL(rollValueChanged(double)), this, SLOT(UpdateCameraRoll(double)));
                                connect(camera_dialog_, SIGNAL(cameraTypeChanged(int)), this, SLOT(ChangeCameraType(int)));
                                connect(camera_dialog_, SIGNAL(sensitivityChanged(float)), this, SLOT(set_sensitivity(float)));

                                connect(this, SIGNAL(CameraAttitudeChanged(QVector3D)), camera_dialog_, SLOT(updateCameraAttitude(QVector3D)));
                                connect(this, SIGNAL(CameraPosChanged(QVector3D)), camera_dialog_, SLOT(updateCameraPos(QVector3D)));
                        }
                }
