...
GrpcClientImpl::GrpcClientImpl(std::shared_ptr<Channel> channel) : stub_(CarToObu::NewStub(channel)) 
{
        AINFO << "GrpcClientImpl initial success";
        carstatus_tv_nsec_ = (1000000000 / static_cast<int>(FLAGS_v2x_carstatus_timer_frequency) / 2);
        init_flag_ = true;
}
...
void GrpcClientImpl::SendMsgToGrpc(const std::shared_ptr<PerceptionObstacles> &msg) 
{
        ...
        {
                ADEBUG << "stub PushPerceptionResult success. time used: " << time_used.count() * 1000 << "ms";
        }
}
