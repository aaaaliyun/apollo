...
GrpcServerImpl::GrpcServerImpl() : node_(cyber::CreateNode("v2x_grpc_server")) 
{
        ...
        first_flag_writer_ = node_->CreateWriter<StatusResponse>("/apollo/v2x/inner/sync_flag");
        CHECK(first_flag_writer_) << "Create sync flag writer failed";
        AINFO << "GrpcServerImpl initial success";
        init_flag_ = true;
}

grpc::Status GrpcServerImpl::SendPerceptionObstacles(grpc::ServerContext* /* context */, const PerceptionObstacles* request, StatusResponse* response) 
{
...
void GrpcServerImpl::GetMsgFromGrpc(const std::shared_ptr<PerceptionObstacles>& ptr) 
{
...
void GrpcServerImpl::GetMsgFromGrpc(const std::shared_ptr<IntersectionTrafficLightData>& ptr) 
{
