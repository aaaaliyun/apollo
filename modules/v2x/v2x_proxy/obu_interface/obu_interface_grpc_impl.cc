...
ObuInterFaceGrpcImpl::ObuInterFaceGrpcImpl() 
{
        ...
        grpc_client_ = std::make_shared<GrpcClientImpl>(grpc::CreateChannel(FLAGS_grpc_client_host + ":" + FLAGS_grpc_client_port, grpc::InsecureChannelCredentials()));
...
bool ObuInterFaceGrpcImpl::InitialServer() 
{
        ...
        thread_ptr_.reset(new std::thread(&ObuInterFaceGrpcImpl::ThreadRunServer, this));
...
void ObuInterFaceGrpcImpl::ThreadRunServer() 
{
        ...
        AINFO << "ObuInterFaceGrpcImpl grpc server has listening on : " << server_address << " time used : " << time_used.count();
        condition_.wait(lck, [&]() { return exit_flag_; });
        // server_->Wait();
        // cyber::Spin();
}

void ObuInterFaceGrpcImpl::GetV2xObstaclesFromObu(const std::shared_ptr<PerceptionObstacles> &msg) 
{
        grpc_server_->GetMsgFromGrpc(msg);
}

void ObuInterFaceGrpcImpl::GetV2xTrafficLightFromObu(const std::shared_ptr<IntersectionTrafficLightData> &msg) 
{
        grpc_server_->GetMsgFromGrpc(msg);
}
...
void ObuInterFaceGrpcImpl::SendObstaclesToObu(const std::shared_ptr<PerceptionObstacles> &msg) 
{
        grpc_client_->SendMsgToGrpc(msg);
}
