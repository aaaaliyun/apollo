...
public:
        ...
        */
        void GetObstaclesFromOs(const std::shared_ptr<apollo::perception::PerceptionObstacles> &msg);

        /*function that send trafficlight msg to apollo os
        @param input trafficlight type msg shared ptr
        */
        void SendV2xTrafficLightToOs(const std::shared_ptr<IntersectionTrafficLightData> &msg);

        /*function that send obstacles msg to apollo os
        @param input obstacles type msg shared ptr
        */
        void SendV2xObstaclesToOs(const std::shared_ptr<apollo::perception::PerceptionObstacles> &msg);
...
private:
        ...
        */
        ...
        void GetMsgFromOs(const cyber::Reader<MessageT> *reader, const std::shared_ptr<MessageT> &msg) 
        {
                node_->Observe();
                if (reader->Empty()) 
                {
        ...
        */
        template <typename MessageT>
        void SendMsgToOs(cyber::Writer<MessageT> *writer, const std::shared_ptr<MessageT> &msg) 
        {
                if (writer == nullptr) 
                {
        ...
        std::shared_ptr<cyber::Reader<apollo::perception::PerceptionObstacles>> perception_obstacle_reader_ = nullptr;
        std::shared_ptr<cyber::Writer<apollo::perception::PerceptionObstacles>> v2x_obstacle_writer_ = nullptr;
        std::shared_ptr<cyber::Writer<IntersectionTrafficLightData>> v2x_trafficlight_writer_ = nullptr;
