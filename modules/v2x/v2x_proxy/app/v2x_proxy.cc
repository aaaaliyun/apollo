    ...
    : node_(cyber::CreateNode("v2x_proxy")), init_flag_(false) 
{
        ...
        auto x2v_trafficlight_timer_period = ceil((1.0 / static_cast<int>(FLAGS_x2v_trafficlight_timer_frequency)) * 1000);
        x2v_trafficlight_timer_.reset(new cyber::Timer(static_cast<uint32_t>(x2v_trafficlight_timer_period), [this]() { this->OnX2vTrafficLightTimer(); }, false));
        auto v2x_carstatus_timer_period = ceil((1.0 / static_cast<int>(FLAGS_v2x_carstatus_timer_frequency)) * 1000);
        v2x_carstatus_timer_.reset(new cyber::Timer(static_cast<uint32_t>(v2x_carstatus_timer_period), [this]() { this->OnV2xCarStatusTimer(); }, false));
        ...
        if (!x2v_trafficlight_timer_ || !v2x_carstatus_timer_ || !os_interface_ || !obu_interface_ || !x2v_trafficlight_ || !v2x_carstatus_) 
        {
        ...
        first_flag_reader_ = node_->CreateReader<StatusResponse>("/apollo/v2x/inner/sync_flag", [this](const std::shared_ptr<const StatusResponse>& msg) 
        {
...
void V2xProxy::OnV2xCarStatusTimer() 
{
        ...
        if (!localization || !localization->has_header() || !localization->has_pose()) 
        {
