...
void VehicleModel::RearCenteredKinematicBicycleModel(const VehicleModelConfig& vehicle_model_config, const double predicted_time_horizon, const VehicleState& cur_vehicle_state, VehicleState* predicted_vehicle_state) 
{
                                   ...
                                   const VehicleState& cur_vehicle_state) 
{
        ...
        CHECK(cyber::common::GetProtoFromFile(FLAGS_vehicle_model_config_filename, &vehicle_model_config)) << "Failed to load vehicle model config file " << FLAGS_vehicle_model_config_filename;

        // Some models not supported for now
        CHECK(vehicle_model_config.model_type() != VehicleModelConfig::COM_CENTERED_DYNAMIC_BICYCLE_MODEL);
        ...
        {
                RearCenteredKinematicBicycleModel(vehicle_model_config, predicted_time_horizon, cur_vehicle_state, &predicted_vehicle_state);
        }
