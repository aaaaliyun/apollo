...
{
        CHECK_GE(points_.size(), 3);
        return std::any_of(line_segments_.begin(), line_segments_.end(), [&](const LineSegment2d &poly_seg) { return poly_seg.IsPointIn(point); });
}
...
bool Polygon2d::Contains(const Polygon2d &polygon) const 
{
        ...
        return std::all_of(line_segments.begin(), line_segments.end(), [&](const LineSegment2d &line_segment) {return Contains(line_segment); });
}
                                  ...
                                  Polygon2d *const polygon) 
{
        ...
        for (int i = 0; i < n + n; ++i) 
        {
                ...
                while (count > last_count && CrossProd(points[results[count - 2]], points[results[count - 1]], pt) <= kMathEpsilon) 
                {
                        results.pop_back();
                        --count;
                }
