...
void NDTLocalization::Init() 
{
        ...
        if (!success) 
        {
                AERROR << "LocalizationLidar: Fail to access the lidar extrinsic file: " << lidar_extrinsics_file;
                return;
        }
...
void NDTLocalization::OdometryStatusCallback(const std::shared_ptr<drivers::gnss::InsStat>& status_msg) 
{
...
void NDTLocalization::ComposeLocalizationEstimate(const Eigen::Affine3d& pose, const std::shared_ptr<localization::Gps>& odometry_msg, LocalizationEstimate* localization) 
{
        localization->Clear();
...
void NDTLocalization::ComposeLidarResult(double time_stamp, const Eigen::Affine3d& pose, LocalizationEstimate* localization) 
{
        localization->Clear();
        FillLocalizationMsgHeader(localization);
...
bool NDTLocalization::QueryPoseFromTF(double time, Eigen::Affine3d* pose) 
{
        ...
        std::string err_msg = "";
        bool can_transform = tf_buffer_->canTransform(tf_target_frame_id_, tf_source_frame_id_, query_time, time_out, &err_msg);
        if (!can_transform) 
        {
        ...
        try 
        {
                query_transform = tf_buffer_->lookupTransform(tf_target_frame_id_, tf_source_frame_id_, query_time);
        } 
...
void NDTLocalization::ComposeLocalizationStatus(const drivers::gnss::InsStat& status, LocalizationStatus* localization_status) 
{
        ...
        if (ndt_score_ < warnning_ndt_score_ && pos_type == drivers::gnss::SolutionType::INS_RTKFIXED) 
        {
                ...
                (pos_type != drivers::gnss::SolutionType::INS_RTKFIXED && pos_type != drivers::gnss::SolutionType::INS_RTKFLOAT)) 
        {
                localization_status->set_fusion_status(MeasureState::ERROR);
...
bool NDTLocalization::QueryPoseFromBuffer(double time, Eigen::Affine3d* pose) 
{
        ...
        common::math::EulerAnglesZXYd next_euler(next_quat.w(), next_quat.x(), next_quat.y(), next_quat.z());
...
void NDTLocalization::LidarMsgTransfer(const std::shared_ptr<drivers::PointCloud>& msg, LidarFrame* lidar_frame) 
{
...
bool NDTLocalization::LoadLidarExtrinsic(const std::string& file_path, Eigen::Affine3d* lidar_extrinsic) 
{
...
bool NDTLocalization::FindNearestOdometryStatus(const double odometry_timestamp, drivers::gnss::InsStat* status) 
{
