...
void LocalizationPoseBuffer::UpdateLidarPose(double timestamp, const Eigen::Affine3d& locator_pose, const Eigen::Affine3d& novatel_pose) 
{
        if (!has_initialized_) 
        {
...
Eigen::Affine3d LocalizationPoseBuffer::UpdateOdometryPose(double timestamp, const Eigen::Affine3d& novatel_pose) 
{
        ...
        if (used_buffer_size_ > 0) 
        {
                ...
                common::math::EulerAnglesZXYd novatel_ev(novatel_quat.w(), novatel_quat.x(), novatel_quat.y(), novatel_quat.z());
                Eigen::Vector3d pose_ev;
                pose_ev[0] = 0;
                pose_ev[1] = 0;
                pose_ev[2] = 0;

                for (unsigned int c = 0, i = head_index_; c < used_buffer_size_; ++c, i = (i + 1) % s_buffer_size_) 
                {
                        predict_pose.translation() = lidar_poses_[i].locator_pose.translation() - lidar_poses_[i].novatel_pose.translation() + novatel_pose.translation();
                        ...
                        Eigen::Quaterniond predict_pose_quat = pair_locator_quat * pair_novatel_quat.inverse() * novatel_quat;
                        predict_pose_quat.normalized();

                        common::math::EulerAnglesZXYd predict_pose_ev(predict_pose_quat.w(), predict_pose_quat.x(), predict_pose_quat.y(), predict_pose_quat.z());
