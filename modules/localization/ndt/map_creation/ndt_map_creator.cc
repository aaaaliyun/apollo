...
int main(int argc, char** argv) 
{
        ...
        boost_desc.add_options()("help", "produce help message")
                                ("pcd_folders", boost::program_options::value<std::vector<std::string>>(), "provide the pcd folders")
                                ("pose_files", boost::program_options::value<std::vector<std::string>>(), "provide pose files")
                                ("map_folder", boost::program_options::value<std::string>(), "provide map folder")
                                ("resolution_type", boost::program_options::value<std::string>(), "map resolution type: single or multi")
                                ("resolution", boost::program_options::value<float>(), "provide map resolution")
                                ("resolution_z", boost::program_options::value<float>()->default_value(0.03125), "provide map resolution of z")
                                ("zone_id", boost::program_options::value<int>(), "provide zone id")
                                ("set_road_cells", boost::program_options::value<bool>()->default_value(true), "whether enable setting road cells, default true")
                                ("pool_size", boost::program_options::value<unsigned int>()->default_value(20), "set pool size");
        ...
        if (strcasecmp(resolution_type.c_str(), "single") != 0 && strcasecmp(resolution_type.c_str(), "multi") != 0) 
        {
        ...
        {
                apollo::localization::msf::velodyne::LoadPcdPoses(pose_files[i], &pcd_poses[i], &time_stamps[i], &pcd_indices[i]);
        }
      ...
      for (size_t k = 0; k < velodyne_frame.pt3ds.size(); ++k) {
        ...
        for (size_t res = 0; res < ndt_map_config.map_resolutions_.size();
          ...
          ndt_map_node->SetIsChanged(true);
                                        ...
                                        float resolution = ndt_map_node->GetMapResolution();
                                        Eigen::Vector3f centroid;
                                        centroid[0] = static_cast<float>(coord2d[0]) - static_cast<float>(left_top_corner[0]) - resolution * static_cast<float>(x);
                                        centroid[1] = static_cast<float>(coord2d[1]) - static_cast<float>(left_top_corner[1]) - resolution * static_cast<float>(y);
                                        apollo::localization::msf::pyramid_map::NdtMapCells& ndt_map_cell = ndt_map_matrix.GetMapCell(y, x);
                                        int altitude_index = ndt_map_cell.CalAltitudeIndex(ndt_map_config.map_resolutions_z_[res], static_cast<float>(pt3d_global[2]));
                                        centroid[2] = static_cast<float>(pt3d_global[2]) - static_cast<float>(altitude_index) * ndt_map_config.map_resolutions_z_[res];

                                        ndt_map_cell.AddSample(intensity, static_cast<float>(pt3d_global[2]), ndt_map_config.map_resolutions_z_[res], centroid, false);
                                        ndt_map_node->SetIsChanged(true);
                                }
                                ...
                                ndt_map_config.map_datasets_.insert(ndt_map_config.map_datasets_.end(), pcd_folder_paths.begin(), pcd_folder_paths.end());
                                char file_buf[1024];
                                ...
                                for (unsigned int i = 0; i < pcd_folder_paths.size(); ++i) 
                                {
                                        ...
                                        for (unsigned int frame_idx = 0; frame_idx < pcd_poses_i.size(); ++frame_idx) 
                                        {
                                                ...
                                                for (size_t k = 0; k < velodyne_frame.pt3ds.size(); ++k) 
                                                {
                                                        ...
                                                        {
                                                                apollo::localization::msf::pyramid_map::MapNodeIndex index = apollo::localization::msf::pyramid_map::MapNodeIndex::
                                                                GetMapNodeIndex(ndt_map_config, pt3d_global, static_cast<unsigned int>(res), zone_id);
