                           ...
                           unsigned int resolution_id, int zone_id) 
{
        ...
        // set filter
        filter_x_ = static_cast<int>(static_cast<float>(FLAGS_ndt_filter_size_x) / map_.GetMapConfig().map_resolutions_[resolution_id_]);
        filter_y_ = static_cast<int>(static_cast<float>(FLAGS_ndt_filter_size_y) / map_.GetMapConfig().map_resolutions_[resolution_id_]);
...
void LidarLocatorNdt::LoadMap(const Eigen::Affine3d& init_location, unsigned int resolution_id, int zone_id) 
{
        map_preload_node_pool_.Initial(&(map_.GetMapConfig()));
        map_.InitMapNodeCaches(12, 24);
        map_.AttachMapNodePool(&map_preload_node_pool_);
...
int LidarLocatorNdt::Update(unsigned int frame_idx, const Eigen::Affine3d& pose, const LidarFrame& lidar_frame) 
{
        ...
        bool map_is_ready = map_.LoadMapArea(center_pose.translation(), resolution_id_, zone_id_, filter_x_, filter_y_);
        ...
        AINFO << "Online Pointcloud size: " << online_points->size() << "/" << online_points_filtered->size();
        ...
        ComposeMapCells(left_top_coord2d, zone_id_, resolution_id_, map_.GetMapConfig().map_resolutions_[resolution_id_], transform.inverse());
    ...
    const Eigen::Affine3d& inverse_transform) 
{
        ...
        MapNodeIndex map_id = MapNodeIndex::GetMapNodeIndex(map_.GetMapConfig(), coord2d, resolution_id, zone_id);
        ...
        int top_center_end_x = static_cast<int>(map_coord_x) - 1 + 2 * static_cast<int>(filter_size_x / 2);
        top_center_end_x = top_center_end_x > static_cast<int>(map_node_size_x) - 1 ? static_cast<int>(map_node_size_x) - 1 : top_center_end_x;
        ...
        int top_right_end_x = 2 * static_cast<int>(filter_size_x / 2) - (top_left_end_x - top_left_start_x + 1) - 1;
        ...
        int middle_left_end_y = static_cast<int>(map_coord_y) - 1 + 2 * static_cast<int>(filter_size_y / 2);
        middle_left_end_y = middle_left_end_y > static_cast<int>(map_node_size_y) - 1 ? static_cast<int>(map_node_size_y) - 1 : middle_left_end_y;
        node_zone.clear();
        ...
        int bottom_left_end_y = 2 * static_cast<int>(filter_size_y / 2) - (top_left_end_y - top_left_start_y + 1) - 1;
        node_zone.clear();
        ...
        Eigen::Vector2d coord2d_center = map_node_lt->GetCoordinate(map_node_size_x / 2, map_node_size_y / 2);
                ...
                for (int x = 0; x < 3; ++x) 
                {
                            ...
                            map_nodes_zones[y * 3 + x][3] - map_nodes_zones[y * 3 + x][1] >= 0) 
                        {
                                ...
                                else 
                                {
                                        ...
                                        coord2d_xy[0] = coord2d_center[0] + static_cast<double>(x * map_node_size_x) * static_cast<double>(map_pixel_resolution);
                                        coord2d_xy[1] = coord2d_center[1] + static_cast<double>(y * map_node_size_y) * static_cast<double>(map_pixel_resolution);

                                        MapNodeIndex map_id = MapNodeIndex::GetMapNodeIndex(map_.GetMapConfig(), coord2d_xy, resolution_id, zone_id);
                                ...
                                for (int map_y = map_nodes_zones[y * 3 + x][1]; map_y <= map_nodes_zones[y * 3 + x][3]; ++map_y) 
                                {
                                        for (int map_x = map_nodes_zones[y * 3 + x][0]; map_x <= map_nodes_zones[y * 3 + x][2]; ++map_x) 
                                        {
                                                ...
                                                if (cell_ndt.cells_.size() > 0) 
                                                {
                                                        for (auto it = cell_ndt.cells_.begin(); it != cell_ndt.cells_.end(); ++it) 
                                                        {
                                                                ...
                                                                if (cell_count >= 6) 
                                                                {
                                                                        ...
                                                                        eigen_point(0) = left_top_corner[0] + map_x * resolution + it->second.centroid_[0];
                                                                        eigen_point(1) = left_top_corner[1] + map_y * resolution + it->second.centroid_[1];
