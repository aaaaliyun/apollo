...
{
        var = std::sqrt(var);
        unsigned int encoded_var = static_cast<unsigned int>(static_cast<float>(var_range_) / (var * static_cast<float>(var_ratio_) + 1.0));
...
unsigned char LossyMapMatrixHandler::EncodeCount(unsigned int count, unsigned int count_range) const 
{
...
size_t LossyMapFullAltMatrixHandler::LoadBinary(const unsigned char* buf, std::shared_ptr<BaseMapMatrix> base_matrix) 
{
        ...
        if (matrix->HasAltitude()) 
        {
                ...
                for (unsigned int r = 0; r < rows; ++r) 
                {
                        for (unsigned int c = 0; c < cols; ++c) 
                        {
                                ...
                                DecodeAltitude(static_cast<uint16_t>(alt), alt_avg_min_, alt_avg_interval_, &(*altitude_matrix)[r][c]);
                        }
                }
                uc_p += 2 * matrix_size;
        }
        ...
        if (matrix->HasGroundAltitude()) 
        {
                ...
                for (unsigned int r = 0; r < rows; ++r) 
                {
                        for (unsigned int c = 0; c < cols; ++c) 
                        {
                                unsigned int alt = static_cast<unsigned int>(p_high[r * cols + c]);
                                alt = alt * 256u + static_cast<unsigned int>(p_low[r * cols + c]);
                                DecodeAltitude(static_cast<uint16_t>(alt), ground_alt_min_, ground_alt_interval_, &(*ground_altitude_matrix)[r][c]);
                        }
                }
                uc_p += 2 * matrix_size;
        }
...
size_t LossyMapFullAltMatrixHandler::CreateBinary(const std::shared_ptr<BaseMapMatrix> base_matrix, unsigned char* buf, size_t buf_size) 
{
        const std::shared_ptr<PyramidMapMatrix> matrix = std::dynamic_pointer_cast<PyramidMapMatrix>(base_matrix);
...
size_t LossyMapFullAltMatrixHandler::GetBinarySize(const std::shared_ptr<BaseMapMatrix> base_matrix) 
{
        const std::shared_ptr<PyramidMapMatrix> matrix = std::dynamic_pointer_cast<PyramidMapMatrix>(base_matrix);
        // assert(matrix->get_resolution_num() > 0);
        if (matrix->GetResolutionNum() == 0) 
        {
                throw "[LossyMapFullAltMatrixHandler::get_binary_size]matrix->get_resolution_num() == 0";
        }
        ...
        target_size += matrix->GetRowsSafe() * matrix->GetColsSafe() * (sizeof(unsigned char) + sizeof(unsigned char) + sizeof(uint16_t) + sizeof(uint16_t) + sizeof(uint16_t));
        return target_size;
}
...
size_t LosslessMapMatrixHandler::LoadBinary(const unsigned char* buf, std::shared_ptr<BaseMapMatrix> base_matrix) 
{
        std::shared_ptr<PyramidMapMatrix> matrix = std::dynamic_pointer_cast<PyramidMapMatrix>(base_matrix);
...
size_t LosslessMapMatrixHandler::CreateBinary(const std::shared_ptr<BaseMapMatrix> base_matrix, unsigned char* buf, size_t buf_size) 
{
        const std::shared_ptr<PyramidMapMatrix> matrix = std::dynamic_pointer_cast<PyramidMapMatrix>(base_matrix);
...
size_t LosslessMapMatrixHandler::GetBinarySize(const std::shared_ptr<BaseMapMatrix> base_matrix) 
{
        ...
        if (matrix->GetResolutionNum() == 0) 
        {
                throw "[LosslessMapMatrixHandler::get_binary_size]matrix->get_resolution_num() == 0";
        }
...
size_t PyramidLossyMapMatrixHandler::LoadBinary(const unsigned char* buf, std::shared_ptr<BaseMapMatrix> base_matrix) 
{
        std::shared_ptr<PyramidMapMatrix> matrix = std::dynamic_pointer_cast<PyramidMapMatrix>(base_matrix);
        ...
        // reset or init matrix
        if (resolution_num == matrix->GetResolutionNum() && ratio == matrix->GetResolutionRatio() && rows == matrix->GetRowsSafe() &&
            cols == matrix->GetColsSafe() && has_intensity == matrix->HasIntensity() &&
            has_intensity_var == matrix->HasIntensityVar() && has_altitude == matrix->HasAltitude() &&
            has_altitude_var == matrix->HasAltitudeVar() && has_ground_altitude == matrix->HasGroundAltitude() &&
            has_count == matrix->HasCount() && has_ground_count == matrix->HasGroundCount()) 
        {
                matrix->Reset();
        } 
        else 
                ...
                if (matrix->HasGroundAltitude()) 
                {
                        ...
                        for (unsigned int r = 0; r < rows; ++r) 
                        {
                                for (unsigned int c = 0; c < cols; ++c) 
                                {
                                        uint16_t alt = p_high[r * cols + c];
                                        alt = static_cast<uint16_t>(alt * 256 + p_low[r * cols + c]);
                                        DecodeAltitude(alt, ground_alt_min_, ground_alt_interval_, &(*ground_altitude_matrix)[r][c]);
                                }
                        }
                        uc_p += 2 * matrix_size;
                }
...
size_t PyramidLossyMapMatrixHandler::CreateBinary(const std::shared_ptr<BaseMapMatrix> base_matrix, unsigned char* buf, size_t buf_size) 
{
        const std::shared_ptr<PyramidMapMatrix> matrix = std::dynamic_pointer_cast<PyramidMapMatrix>(base_matrix);
...
size_t PyramidLossyMapMatrixHandler::GetBinarySize(const std::shared_ptr<BaseMapMatrix> base_matrix) 
{
        const std::shared_ptr<PyramidMapMatrix> matrix = std::dynamic_pointer_cast<PyramidMapMatrix>(base_matrix);
        ...
        size_t target_size = sizeof(unsigned int) * 2 + sizeof(unsigned int) * 2 + sizeof(unsigned char) * 4 + sizeof(float) * 2;
...
size_t PyramidLosslessMapMatrixHandler::LoadBinary(const unsigned char* buf, std::shared_ptr<BaseMapMatrix> base_matrix) 
{
        std::shared_ptr<PyramidMapMatrix> matrix = std::dynamic_pointer_cast<PyramidMapMatrix>(base_matrix);
        ...
        {
                matrix->Init(rows, cols, has_intensity, has_intensity_var, has_altitude, has_altitude_var, has_ground_altitude, has_count, has_ground_count, resolution_num, ratio);
        }
...
size_t PyramidLosslessMapMatrixHandler::CreateBinary(const std::shared_ptr<BaseMapMatrix> base_matrix, unsigned char* buf, size_t buf_size) 
{
        const std::shared_ptr<PyramidMapMatrix> matrix = std::dynamic_pointer_cast<PyramidMapMatrix>(base_matrix);
...
size_t PyramidLosslessMapMatrixHandler::GetBinarySize(const std::shared_ptr<BaseMapMatrix> base_matrix) 
{
        const std::shared_ptr<PyramidMapMatrix> matrix = std::dynamic_pointer_cast<PyramidMapMatrix>(base_matrix);
        ...
        if (resolution_num == 0) 
        {
                throw "[PyramidLosslessMapMatrixHandler::get_binary_size]resolution_num == 0";
        }

        // resolution_num and ratio
        // rows and cols in first level
        // space for has_*
        size_t target_size = sizeof(unsigned int) * 2 + sizeof(unsigned int) * 2 + sizeof(unsigned char) * 4;
