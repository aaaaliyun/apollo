        ...
        *intensity_img = cv::Mat(cv::Size(cols_mr_[level], rows_mr_[level]), CV_8UC1);
        ...
        for (unsigned int y = 0; y < rows_mr_[level]; ++y) 
        {
                for (unsigned int x = 0; x < cols_mr_[level]; ++x) 
                {
                        ...
                        {
                                intensity_img->at<unsigned char>(y, x) = static_cast<unsigned char>(intensity_matrixes_[level][y][x]);
                        }
                }
        }
        ...
        for (unsigned int y = 0; y < rows_mr_[level]; y++) 
        {
                for (unsigned int x = 0; x < cols_mr_[level]; x++) 
                {
                        if (count_matrixes_[level][y][x] > 0) 
                        {
                                ...
                                {
                                        altitude_img->at<unsigned char>(y, x) = static_cast<unsigned char>((altitude_matrixes_[level][y][x] - min_alt) / (max_alt - min_alt) * 255);
                                }
                        } 
                        else 
                        {
                                ...
                                for (unsigned int rl = r0; rl < r0 + ratio_; ++rl) 
                                {
                                        for (unsigned int cl = c0; cl < c0 + ratio_; ++cl) 
                                        {
                                                ...
                                                if (count > 0) 
                                                {
                                                        ...
                                                        GetMapCellSafe(&intensity, &intensity_var, &altitude, &altitude_var, &ground_altitude, &count, &ground_count, rl, cl, i - 1);
                                                        MergeCellSafe(intensity, intensity_var, altitude, altitude_var, ground_altitude, count, ground_count, r, c, i);
                                                }
                                        }
                                }
                        }
...
const float* PyramidMapMatrix::GetIntensitySafe(unsigned int row, unsigned int col, unsigned int level) const 
{
        if (!has_intensity_) 
        {
...
const float* PyramidMapMatrix::GetIntensityVarSafe(unsigned int row, unsigned int col, unsigned int level) const 
{
        if (!has_intensity_var_) 
        {
...
const float* PyramidMapMatrix::GetAltitudeSafe(unsigned int row, unsigned int col, unsigned int level) const 
{
        if (!has_altitude_) 
        {
...
const float* PyramidMapMatrix::GetAltitudeVarSafe(unsigned int row, unsigned int col, unsigned int level) const 
{
        if (!has_altitude_var_) 
        {
...
const float* PyramidMapMatrix::GetGroundAltitudeSafe(unsigned int row, unsigned int col, unsigned int level) const 
{
        if (!has_ground_altitude_) 
        {
...
const unsigned int* PyramidMapMatrix::GetCountSafe(unsigned int row, unsigned int col, unsigned int level) const 
{
        if (!has_count_) 
        {
...
const unsigned int* PyramidMapMatrix::GetGroundCountSafe(unsigned int row, unsigned int col, unsigned int level) const 
{
        if (!has_ground_count_) 
        {
...
FloatMatrix* PyramidMapMatrix::GetIntensityMatrixSafe(unsigned int level) 
{
        ...
        if (level >= resolution_num_) 
        {
                AERROR << "PyramidMapMatrix: [GetIntensityMatrixSafe] The level id is illegal.";
                return nullptr;
        }
...
FloatMatrix* PyramidMapMatrix::GetIntensityVarMatrixSafe(unsigned int level) 
{
        if (!has_intensity_var_) 
        {
                AERROR << "PyramidMapMatrix: [GetIntensityVarMatrixSafe] Has no intensity_var.";
                return nullptr;
        }

        if (level >= resolution_num_) 
        {
                AERROR << "PyramidMapMatrix: [GetIntensityVarMatrixSafe] The level id is illegal.";
                return nullptr;
        }
...
FloatMatrix* PyramidMapMatrix::GetAltitudeVarMatrixSafe(unsigned int level) 
{
        ...
        if (level >= resolution_num_) 
        {
                AERROR << "PyramidMapMatrix: [GetAltitudeVarMatrixSafe] The level id is illegal.";
                return nullptr;
        }
...
FloatMatrix* PyramidMapMatrix::GetGroundAltitudeMatrixSafe(unsigned int level) 
{
        if (!has_ground_altitude_) 
        {
                AERROR << "PyramidMapMatrix: [GetGroundAltitudeMatrixSafe] Has no ground_altitude.";
                return nullptr;
        }

        if (level >= resolution_num_) 
        {
                AERROR << "PyramidMapMatrix: [GetGroundAltitudeMatrixSafe] The level id is illegal.";
                return nullptr;
        }
...
UIntMatrix* PyramidMapMatrix::GetGroundCountMatrixSafe(unsigned int level) 
{
        ...
        if (level >= resolution_num_) 
        {
                AERROR << "PyramidMapMatrix: [GetGroundCountMatrixSafe] The level id is illegal.";
                return nullptr;
        }
...
void PyramidMapMatrix::SetIntensityMatrix(const float* input, unsigned int size, unsigned int start_index, unsigned int level) 
{
        if (!has_intensity_) 
        {
...
void PyramidMapMatrix::SetIntensityVarMatrix(const float* input, unsigned int size, unsigned int start_index, unsigned int level) 
{
        if (!has_intensity_var_) 
        {
...
void PyramidMapMatrix::SetAltitudeMatrix(const float* input, unsigned int size, unsigned int start_index, unsigned int level) 
{
        if (!has_altitude_) 
        {
...
void PyramidMapMatrix::SetAltitudeVarMatrix(const float* input, unsigned int size, unsigned int start_index, unsigned int level) 
{
        if (!has_altitude_var_) 
        {
...
void PyramidMapMatrix::SetGroundAltitudeMatrix(const float* input, unsigned int size, unsigned int start_index, unsigned int level) 
{
        if (!has_ground_altitude_) 
        {
                AERROR << "PyramidMapMatrix: [SetGroundAltitudeMatrix] Has no ground_altitude.";
                return;
        }
...
void PyramidMapMatrix::SetCountMatrix(const unsigned int* input, unsigned int size, unsigned int start_index, unsigned int level) 
{
        if (!has_count_) 
        {
...
void PyramidMapMatrix::SetGroundCountMatrix(const unsigned int* input, unsigned int size, unsigned int start_index, unsigned int level) 
{
        if (!has_ground_count_) 
        {
...
void PyramidMapMatrix::SetFloatMatrixRoi(const FloatMatrix* source_matrix, const Rect2D<unsigned int>& source_roi, const Rect2D<unsigned int>& target_roi, unsigned int type, unsigned int level) 
{
        ...
        if (!CheckLegalityForSetDataRoi( level, static_cast<unsigned int>(source_matrix->GetRow()), static_cast<unsigned int>(source_matrix->GetCol()), source_roi, target_roi)) 
        {
        ...
        for (unsigned int y = target_roi_min_y; y <= target_roi_max_y; ++y) 
        {
                ...
                unsigned int source_start_index = (source_roi_min_y + inc) * source_matrix->GetCol() + source_roi_min_x;
...
void PyramidMapMatrix::SetIntensitySafe(float intensity, unsigned int row, unsigned int col, unsigned int level) 
{
        if (!has_intensity_) 
        {
...
void PyramidMapMatrix::SetIntensityVarSafe(float intensity_var, unsigned int row, unsigned int col, unsigned int level) 
{
        if (!has_intensity_var_) 
        {
...
void PyramidMapMatrix::SetAltitudeSafe(float altitude, unsigned int row, unsigned int col, unsigned int level) 
{
        if (!has_altitude_) 
        {
...
void PyramidMapMatrix::SetAltitudeVarSafe(float altitude_var, unsigned int row, unsigned int col, unsigned int level) 
{
        if (!has_altitude_var_) 
        {
...
void PyramidMapMatrix::SetGroundAltitudeSafe(float ground_altitude, unsigned int row, unsigned int col, unsigned int level) 
{
        if (!has_ground_altitude_) 
        {
...
void PyramidMapMatrix::SetCountSafe(unsigned int count, unsigned int row, unsigned int col, unsigned int level) 
{
        if (!has_count_) 
        {
...
void PyramidMapMatrix::SetGroundCountSafe(unsigned int ground_count, unsigned int row, unsigned int col, unsigned int level) 
{
        if (!has_ground_count_) 
        {
...
void PyramidMapMatrix::SetValueSafe(unsigned char intensity, float altitude, unsigned int row, unsigned int col, unsigned int level) 
{
        if (!has_intensity_) 
        {
    ...
    unsigned int row, unsigned int col, unsigned int level) 
{
        ...
        if (intensity != nullptr && has_intensity_) 
        {
                ...
                intensity_matrixes_[level][row][col] = intensity_matrixes_[level][row][col] * p0 + *intensity * p1;
        }

        if (intensity != nullptr && has_intensity_ && intensity_var != nullptr && has_intensity_var_) 
        {
                intensity_var_matrixes_[level][row][col] = intensity_var_matrixes_[level][row][col] * p0 + *intensity_var * p1 +
        ...
        {
                altitude_var_matrixes_[level][row][col] = altitude_var_matrixes_[level][row][col] * p0 + *altitude_var * p1 + altitude_diff * altitude_diff * p0 * p1;
        }
        ...
        p0 = static_cast<float>(ground_count_matrixes_[level][row][col]) / static_cast<float>(new_ground_count);
        p1 = static_cast<float>(*ground_count) / static_cast<float>(new_ground_count);

        if (ground_altitude != nullptr && has_ground_altitude_) 
        {
                ground_altitude_matrixes_[level][row][col] = ground_altitude_matrixes_[level][row][col] * p0 + *ground_altitude * p1;
        }

        ground_count_matrixes_[level][row][col] = new_ground_count;
}

bool PyramidMapMatrix::CheckLegalityForGetData(unsigned int row, unsigned int col, unsigned int level) const 
{
        if (level >= resolution_num_) 
        {
                AERROR << "PyramidMapMatrix: [CheckLegalityForGetData] The level id is illegal.";
                return false;
        }
...
bool PyramidMapMatrix::CheckLegalityForSetData(unsigned int level, unsigned int start_id, unsigned int size) const 
{
        if (level >= resolution_num_) 
        {
                AERROR << "PyramidMapMatrix: [CheckLegalityForSetData] The level id is illegal.";
                return false;
        }

        if (start_id + size > rows_mr_[level] * cols_mr_[level]) 
        {
                AERROR << "PyramidMapMatrix: [CheckLegalityForSetData] The start_id or size is illegal.";
                return false;
        }
    ...
    const Rect2D<unsigned int>& target_roi) const 
{
        if (level >= resolution_num_) 
        {
                AERROR << "PyramidMapMatrix: [CheckLegalityForSetDataRoi] The level id is illegal.";
                return false;
        }
        ...
        {
                AERROR << "PyramidMapMatrix: [CheckLegalityForSetDataRoi] The source_roi or target_roi is illegal.";
                return false;
        }
...
void PyramidMapMatrix::AddSampleSafe(float intensity, float altitude, unsigned int row, unsigned int col, unsigned int level) 
{
        if (!CheckLegalityForGetData(row, col, level)) 
        {
        ...
        if (has_count_ && has_intensity_ && has_intensity_var_) 
        {
                ...
                intensity_var_matrixes_[level][row][col] = (static_cast<float>(count_matrixes_[level][row][col] - 1) * intensity_var_matrixes_[level][row][col] + v1 * v2) / static_cast<float>(count_matrixes_[level][row][col]);
        }
        ...
        if (has_count_ && has_altitude_ && has_altitude_var_) 
        {
                ...
                altitude_var_matrixes_[level][row][col] = (static_cast<float>(count_matrixes_[level][row][col] - 1) * altitude_var_matrixes_[level][row][col] + v1 * v2) / static_cast<float>(count_matrixes_[level][row][col]);
        }
}

void PyramidMapMatrix::AddGroundSample(float ground_altitude, unsigned int row, unsigned int col, unsigned int level) 
{
        if (!CheckLegalityForGetData(row, col, level)) 
        {
...
void PyramidMapMatrix::Reduce(std::shared_ptr<PyramidMapMatrix> cells, const PyramidMapMatrix& new_cells, unsigned int level, unsigned int new_level) 
{
        ...
        if (cells->rows_mr_[level] != new_cells.rows_mr_[new_level] || cells->cols_mr_[level] != new_cells.cols_mr_[new_level]) 
        {
                return;
        }
        ...
        for (unsigned int r = 0; r < cells->rows_mr_[level]; r++) 
        {
                for (unsigned int c = 0; c < cells->cols_mr_[level]; c++) 
                {
                        ...
                        cells->MergeCellSafe(intensity, intensity_var, altitude, altitude_var, ground_altitude, count, ground_count, r, c, level);
                }
        }
}
