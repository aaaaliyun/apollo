...
void PyramidMapNode::Init(const BaseMapConfig* map_config) 
{
        ...
        map_matrix_handler_.reset(PyramidMapMatrixHandlerSelector::AllocPyramidMapMatrixHandler(map_node_config_->map_version_));
        ...
        {
                resolutions_mr_[i] = static_cast<float>(pm_map_config->map_resolutions_[0] * std::pow(pm_map_config->resolution_ratio_, i));
        }
                          ...
                          const MapNodeIndex& index, bool create_map_cells) 
{
        ...
        map_node_config_->map_version_ = map_config_->GetMapVersion();
        if (map_node_config_->map_version_ == MapVersion::LOSSY_FULL_ALT_MAP || map_node_config_->map_version_ == MapVersion::LOSSLESS_MAP) 
        {
                map_node_config_->has_map_version_ = false;
                map_node_config_->has_body_md5_ = false;
        } 
...
{
        std::shared_ptr<PyramidMapMatrix> map_matrix = std::dynamic_pointer_cast<PyramidMapMatrix>(map_matrix_);
                                       ...
                                       unsigned int level) 
{
        ...
        {
                std::shared_ptr<PyramidMapMatrix> map_matrix = std::dynamic_pointer_cast<PyramidMapMatrix>(map_matrix_);
                map_matrix->AddSampleBase(static_cast<float>(intensity), static_cast<float>(coordinate[2]), y, x, level);
                return true;
        } 
        else 
        {
                                   ...
                                   unsigned int* y) const 
{
        ...
        unsigned int off_x = static_cast<unsigned int>((coordinate[0] - left_top_corner_[0]) / current_resolution);
        unsigned int off_y = static_cast<unsigned int>((coordinate[1] - left_top_corner_[1]) / current_resolution);
                                              ...
                                              unsigned int y) const 
{
        ...
        float current_resolution = static_cast<float>(map_config->map_resolutions_[0] * std::pow(map_config->resolution_ratio_, level));
...
bool PyramidMapNode::GetCoordinate(const Eigen::Vector2d& coordinate, unsigned int* x, unsigned int* y) const 
{
        return BaseMapNode::GetCoordinate(coordinate, x, y);
}

bool PyramidMapNode::GetCoordinate(const Eigen::Vector3d& coordinate, unsigned int* x, unsigned int* y) const 
{
        return BaseMapNode::GetCoordinate(coordinate, x, y);
}
...
float PyramidMapNode::GetIntensitySafe(const Eigen::Vector3d& coordinate, unsigned int level) const 
{
        Eigen::Vector2d coord2d(coordinate[0], coordinate[1]);
        unsigned int x = 0;
        unsigned int y = 0;
...
float PyramidMapNode::GetIntensityVarSafe(const Eigen::Vector3d& coordinate, unsigned int level) const 
{
        Eigen::Vector2d coord2d(coordinate[0], coordinate[1]);
        unsigned int x = 0;
        unsigned int y = 0;
...
float PyramidMapNode::GetGroundAltitudeSafe(const Eigen::Vector3d& coordinate, unsigned int level) const 
{
        Eigen::Vector2d coord2d(coordinate[0], coordinate[1]);
        unsigned int x = 0;
        unsigned int y = 0;
...
unsigned int PyramidMapNode::GetGroundCountSafe(const Eigen::Vector3d& coordinate, unsigned int level) const 
{
        Eigen::Vector2d coord2d(coordinate[0], coordinate[1]);
        unsigned int x = 0;
        unsigned int y = 0;
...
float PyramidMapNode::GetGroundAltitude(const Eigen::Vector3d& coordinate, unsigned int level) const 
{
        Eigen::Vector2d coord2d(coordinate[0], coordinate[1]);
        unsigned int x = 0;
        unsigned int y = 0;
...
unsigned int PyramidMapNode::GetGroundCount(const Eigen::Vector3d& coordinate, unsigned int level) const 
{
        Eigen::Vector2d coord2d(coordinate[0], coordinate[1]);
        unsigned int x = 0;
        unsigned int y = 0;
