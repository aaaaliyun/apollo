...
{
        map_node_cache_lvl1_.reset(new MapNodeCacheL1<MapNodeIndex, BaseMapNode>(cacheL1_size));
        map_node_cache_lvl2_.reset(new MapNodeCacheL2<MapNodeIndex, BaseMapNode>(cahceL2_size));
}
                             ...
                             unsigned int resolution_id, unsigned int zone_id) 
{
        ...
        pt_top_right[0] = location[0] + (static_cast<float>(this->map_config_->map_node_size_x_) * map_pixel_resolution / 2.0f);
                          ...
                          int filter_size_x, int filter_size_y) 
{
        ...
        /// top left
        Eigen::Vector3d pt_top_left;
        pt_top_left[0] = seed_pt3d[0] - (static_cast<float>(this->map_config_->map_node_size_x_) * map_pixel_resolution / 2.0) - static_cast<float>(filter_size_x / 2) * map_pixel_resolution;
        pt_top_left[1] = seed_pt3d[1] - (static_cast<float>(this->map_config_->map_node_size_y_) * map_pixel_resolution / 2.0) - static_cast<float>(filter_size_y / 2) * map_pixel_resolution;
        pt_top_left[2] = 0;
        MapNodeIndex map_id = MapNodeIndex::GetMapNodeIndex(*map_config_, pt_top_left, resolution_id, zone_id);
        map_ids.insert(map_id);
        /// top center
        ...
        /// top right
        Eigen::Vector3d pt_top_right;
        pt_top_right[0] = seed_pt3d[0] + (static_cast<float>(this->map_config_->map_node_size_x_) * map_pixel_resolution / 2.0) + static_cast<float>(filter_size_x / 2) * map_pixel_resolution;
        pt_top_right[1] = pt_top_left[1];
        pt_top_left[2] = 0;
        map_id = MapNodeIndex::GetMapNodeIndex(*map_config_, pt_top_right, resolution_id, zone_id);
        map_ids.insert(map_id);
        ...
        Eigen::Vector3d pt_bottom_left;
        pt_bottom_left[0] = pt_top_left[0];
        pt_bottom_left[1] = seed_pt3d[1] + (static_cast<float>(this->map_config_->map_node_size_y_) * map_pixel_resolution / 2.0) + static_cast<float>(filter_size_y / 2) * map_pixel_resolution;
        pt_bottom_left[2] = 0;
        map_id = MapNodeIndex::GetMapNodeIndex(*map_config_, pt_bottom_left, resolution_id, zone_id);
        map_ids.insert(map_id);
        /// bottom center
...
MapNodeIndex BaseMap::GetMapIndexFromMapPath(const std::string& map_path) 
{
        MapNodeIndex index;
        char buf[100];
        sscanf(map_path.c_str(), "/%03u/%05s/%02d/%08u/%08u", &index.resolution_id_, buf, &index.zone_id_, &index.m_, &index.n_);
        std::string zone = buf;
        if (zone == "south") 
        {
