...
BaseMapNode::BaseMapNode() : map_config_(NULL), map_matrix_(NULL), map_matrix_handler_(NULL), compression_strategy_(NULL) 
{
        is_changed_ = false;
...
BaseMapNode::BaseMapNode(BaseMapMatrix* matrix, CompressionStrategy* strategy) : map_config_(NULL), map_matrix_(matrix), map_matrix_handler_(NULL), compression_strategy_(strategy) 
{
        is_changed_ = false;
        data_is_ready_ = false;
        is_reserved_ = false;
...
size_t BaseMapNode::LoadHeaderBinary(const unsigned char* buf) 
{
        ...
        if (node_config_tem->map_version_ != map_node_config_->map_version_ || node_config_tem->node_index_ != map_node_config_->node_index_) 
        {
                return 0;
        }
...
size_t BaseMapNode::CreateBodyBinary(std::vector<unsigned char>* buf) const 
{
        ...
        size_t binary_size = map_matrix_handler_->CreateBinary(map_matrix_, &buf_uncompressed[0], body_size);
        if (binary_size == 0) 
        {
                return 0;
        }
...
bool BaseMapNode::GetCoordinate(const Eigen::Vector2d& coordinate, unsigned int* x, unsigned int* y) const 
{
        const Eigen::Vector2d& left_top_corner = GetLeftTopCorner();
        unsigned int off_x = static_cast<unsigned int>((coordinate[0] - left_top_corner[0]) / GetMapResolution());
        unsigned int off_y = static_cast<unsigned int>((coordinate[1] - left_top_corner[1]) / GetMapResolution());
...
{
        const Eigen::Vector2d& left_top_corner = GetLeftTopCorner();
        Eigen::Vector2d coord(left_top_corner[0] + static_cast<float>(x) * GetMapResolution(), left_top_corner[1] + static_cast<float>(y) * GetMapResolution());
        return coord;
}
...
Eigen::Vector2d BaseMapNode::ComputeLeftTopCorner(const BaseMapConfig& config, const MapNodeIndex& index) 
{
        Eigen::Vector2d coord;
        coord[0] = config.map_range_.GetMinX() + static_cast<float>(config.map_node_size_x_) * config.map_resolutions_[index.resolution_id_] * static_cast<float>(index.n_);
        coord[1] = config.map_range_.GetMinY() + static_cast<float>(config.map_node_size_y_) * config.map_resolutions_[index.resolution_id_] * static_cast<float>(index.m_);
        if (coord[0] >= config.map_range_.GetMaxX()) 
        {
                throw "[BaseMapNode::ComputeLeftTopCorner] coord[0] >= config.map_range_.GetMaxX()";
        }
        if (coord[1] >= config.map_range_.GetMaxY()) 
        {
                throw "[BaseMapNode::compute_left_top_corner] coord[1] >= config.map_range_.GetMaxY()";
        }
        return coord;
}
...
bool BaseMapNode::SaveIntensityImage() const 
{
        ...
        snprintf(buf, sizeof(buf), "/%08d.png", abs(map_node_config_->node_index_.n_));
        path = path + buf;
