...
Status MeasureRepublishProcess::Init(const LocalizationIntegParam& params) 
{
        ...
        if (imu_ant_fin) 
        {
                bool success = LoadImuGnssAntennaExtrinsic(params.ant_imu_leverarm_file, &imu_gnssant_extrinsic_);
                if (!success) 
                {
                        AERROR << "IntegratedLocalization: Fail to access the lever arm between imu and gnss extrinsic file: "
        ...
        imu_gnssant_extrinsic_.transform_1.translation()[0] = dcm[0][0] * lever_arm_x + dcm[0][1] * lever_arm_y + dcm[0][2] * lever_arm_z;
        imu_gnssant_extrinsic_.transform_1.translation()[1] = dcm[1][0] * lever_arm_x + dcm[1][1] * lever_arm_y + dcm[1][2] * lever_arm_z;
        imu_gnssant_extrinsic_.transform_1.translation()[2] = dcm[2][0] * lever_arm_x + dcm[2][1] * lever_arm_y + dcm[2][2] * lever_arm_z;
        ...
        imu_gnssant_extrinsic_.transform_2.translation()[0] = dcm[0][0] * lever_arm_x + dcm[0][1] * lever_arm_y + dcm[0][2] * lever_arm_z;
        imu_gnssant_extrinsic_.transform_2.translation()[1] = dcm[1][0] * lever_arm_x + dcm[1][1] * lever_arm_y + dcm[1][2] * lever_arm_z;
        imu_gnssant_extrinsic_.transform_2.translation()[2] = dcm[2][0] * lever_arm_x + dcm[2][1] * lever_arm_y + dcm[2][2] * lever_arm_z;
...
bool MeasureRepublishProcess::NovatelBestgnssposProcess(const GnssBestPose& bestgnsspos_msg, MeasureData* measure) 
{
...
bool MeasureRepublishProcess::LidarLocalProcess(const LocalizationEstimate& lidar_local_msg, MeasureData* measure) 
{
        CHECK_NOTNULL(measure);
        ...
        apollo::localization::msf::FrameTransform::UtmXYToLatlon(lidar_local_msg.pose().position().x(), lidar_local_msg.pose().position().y(), local_utm_zone_id_, false, &temp_wgs);
...
bool MeasureRepublishProcess::CalculateVelFromBestgnsspose(const GnssBestPose& bestgnsspos_msg, MeasureData* measure) 
{
        CHECK_NOTNULL(measure);
        ...
        measure->gnss_vel.ve = (measure->gnss_pos.longitude - pre_bestgnsspose_.gnss_pos.longitude) * rn * inv_time * cos(measure->gnss_pos.latitude);
        measure->gnss_vel.vn = (measure->gnss_pos.latitude - pre_bestgnsspose_.gnss_pos.latitude) * rm * inv_time;
...
bool MeasureRepublishProcess::GnssHeadingProcess(const drivers::gnss::Heading& heading_msg, MeasureData* measure_data, int* status) 
{
        if ((imu_gnssant_extrinsic_.ant_num == 1)) 
        {
                return false;
        ...
        *status = position_type;
        ...
        {
                imu_ant_yaw_angle = -atan2(imu_gnssant_extrinsic_.transform_2.translation()[0] - imu_gnssant_extrinsic_.transform_1.translation()[0], imu_gnssant_extrinsic_.transform_2.translation()[1] - imu_gnssant_extrinsic_.transform_1.translation()[1]) * 57.295779513082323;
...
bool MeasureRepublishProcess::LoadImuGnssAntennaExtrinsic(std::string file_path, VehicleGnssAntExtrinsic* extrinsic) const 
{
        ...
        if (confige["leverarm"]) 
        {
                ...
                {
                        extrinsic->transform_1.translation()[0] = confige["leverarm"]["primary"]["offset"]["x"].as<double>();
                        extrinsic->transform_1.translation()[1] = confige["leverarm"]["primary"]["offset"]["y"].as<double>();
                        extrinsic->transform_1.translation()[2] = confige["leverarm"]["primary"]["offset"]["z"].as<double>();
                } 
                else 
                {
                        return false;
                ...
                {
                        extrinsic->transform_2.translation()[0] = confige["leverarm"]["secondary"]["offset"]["x"].as<double>();
                        extrinsic->transform_2.translation()[1] = confige["leverarm"]["secondary"]["offset"]["y"].as<double>();
                        extrinsic->transform_2.translation()[2] = confige["leverarm"]["secondary"]["offset"]["z"].as<double>();
                        extrinsic->ant_num = 2;
                        ...
                        {
                                double yaw = atan2(extrinsic->transform_2.translation()[0] - extrinsic->transform_1.translation()[0], extrinsic->transform_2.translation()[1] - extrinsic->transform_1.translation()[1]);
                                double quat[4] = {0.0};
                                math::EulerToQuaternion(0.0, 0.0, yaw, quat);
                                extrinsic->transform_1.linear() = Eigen::Quaterniond(quat[0], quat[1], quat[2], quat[3]).toRotationMatrix();
                        }
                        return true;
                } 
                else 
