                             ...
                             const unsigned int resolution_id) 
{
        ...
        lidar_locator_->Init(search_range_x_, search_range_y_, static_cast<float>(resolution_), node_size_x_, node_size_y_);
        return true;
}
...
void LocalizationLidar::SetVelodyneExtrinsic(const Eigen::Affine3d& pose) 
{
        ...
        lidar_locator_->SetVelodyneExtrinsic(trans.x(), trans.y(), trans.z(), quat.x(), quat.y(), quat.z(), quat.w());
}
                              ...
                              const LidarFrame& lidar_frame, bool use_avx) 
{
        ...
        if (!is_map_loaded_) 
        {
                map_.LoadMapArea(pose.translation(), resolution_id_, zone_id_, search_range_x_, search_range_y_);
                is_map_loaded_ = true;
        ...
        lidar_locator_->SetPointCloudData(size, lidar_frame.pt_xs.data(), lidar_frame.pt_ys.data(), lidar_frame.pt_zs.data(), lidar_frame.intensities.data());

        // compute
        int error = lidar_locator_->Compute(pose_trans(0), pose_trans(1), pose_trans(2), pose_quat.x(), pose_quat.y(), pose_quat.z(), pose_quat.w(), use_avx);
...
void LocalizationLidar::RefineAltitudeFromMap(Eigen::Affine3d* pose) 
{
        ...
        MapNodeIndex index = MapNodeIndex::GetMapNodeIndex(map_.GetMapConfig(), lidar_trans, resolution_id_, zone_id_);
...
void LocalizationLidar::ComposeMapNode(const Eigen::Vector3d& trans) 
{
        ...
        // top left corner
        map_node_idx[0][0] = MapNodeIndex::GetMapNodeIndex(map_.GetMapConfig(), left_top_corner, resolution_id_, zone_id_);
        // top right corner
        ...
        for (int i = 0; i < 2; ++i) 
        {
                for (int j = 0; j < 2; ++j) 
                {
                        ...
                        PyramidMapMatrix& map_cells = static_cast<PyramidMapMatrix&>(map_node[i][j]->GetMapCellMatrix());
