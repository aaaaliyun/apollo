...
void LocalizationIntegImpl::PcdProcessImpl(const LidarFrame& pcd_data) 
{
        ...
        if (state == 2) 
        {  
                // only state OK republish lidar msg
        ...
        lastest_lidar_localization_ = LocalizationResult(LocalizationMeasureState(state), lidar_localization);
}
...
void LocalizationIntegImpl::ImuProcessImpl(const ImuData& imu_data) 
{
        ...
        expert_.GetFusionStatus(integ_localization.mutable_msf_status(), integ_localization.mutable_sensor_status(), &integ_status);
        ...
        Eigen::Vector3d orig_angular_velocity(corrected_imu.wibb[0], corrected_imu.wibb[1], corrected_imu.wibb[2]);
        Eigen::Vector3d vec_angular_velocity = static_cast<Eigen::Vector3d>(quaternion.toRotationMatrix() * orig_angular_velocity);
        ...
        lastest_integ_localization_ = LocalizationResult(LocalizationMeasureState(static_cast<int>(state)), integ_localization, integ_status);
...
void LocalizationIntegImpl::RawObservationProcess(const drivers::gnss::EpochObservation& raw_obs_msg) 
{
        if (is_use_gnss_bestpose_) 
        {
                return;
...
void LocalizationIntegImpl::RawEphemerisProcess(const drivers::gnss::GnssEphemeris& gnss_orbit_msg) 
{
        if (is_use_gnss_bestpose_) 
        {
                return;
...
void LocalizationIntegImpl::GnssBestPoseProcess(const drivers::gnss::GnssBestPose& bestgnsspos_msg) 
{
        if (!is_use_gnss_bestpose_) 
        {
                return;
...
void LocalizationIntegImpl::RawObservationProcessImpl(const drivers::gnss::EpochObservation& raw_obs_msg) 
{
...
void LocalizationIntegImpl::RawEphemerisProcessImpl(const drivers::gnss::GnssEphemeris& gnss_orbit_msg) 
{
        gnss_process_->RawEphemerisProcess(gnss_orbit_msg);
}

void LocalizationIntegImpl::GnssBestPoseProcessImpl(const drivers::gnss::GnssBestPose& bestgnsspos_msg) 
{
        MeasureData measure;
        if (republish_process_->NovatelBestgnssposProcess(bestgnsspos_msg, &measure)) 
        {
                ...
                lastest_gnss_localization_ = LocalizationResult(LocalizationMeasureState::OK, gnss_localization);
        }
}
...
void LocalizationIntegImpl::TransferGnssMeasureToLocalization(const MeasureData& measure, LocalizationEstimate* localization) 
{
