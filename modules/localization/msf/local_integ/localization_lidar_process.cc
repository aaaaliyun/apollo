...
Status LocalizationLidarProcess::Init(const LocalizationIntegParam& params) 
{
        ...
        if (!success) 
        {
                AERROR << "LocalizationLidar: Fail to access the lidar extrinsic file: " << lidar_extrinsic_file_;
        ...
        if (!success) 
        {
                AWARN << "LocalizationLidar: Fail to load the lidar height file: " << lidar_height_file_ << " Will use default value!";
...
double LocalizationLidarProcess::ComputeDeltaYawLimit(const int64_t index_cur, const int64_t index_stable, const double limit_min, const double limit_max) 
{
...
void LocalizationLidarProcess::PcdProcess(const LidarFrame& lidar_frame) 
{
        ...
        if (!GetPredictPose(lidar_frame.measurement_time, &cur_predict_location_, &forecast_integ_state_)) 
        {
        ...
        locator_->SetDeltaYawLimit(ComputeDeltaYawLimit(forecast_timer_, 10, delta_yaw_limit_, init_delta_yaw_limit_));
        ...
        int ret = locator_->Update(pcd_index++, cur_predict_location_, velocity_, lidar_frame, if_use_avx_);
                                              ...
                                              ForecastState* forecast_state) 
{
        ...
        else 
        {
                ...
                state = pose_forecastor_->GetBestForcastPose(lidar_time, pre_location_time_, init_pose, &forecast_pose);
        }
...
void LocalizationLidarProcess::UpdateState(const int ret, const double time) 
{
        ...
        {  
                ...
                // check covariance
                double cur_location_std_area = std::sqrt(local_uncertainty_x * local_uncertainty_x + local_uncertainty_y * local_uncertainty_y);
...
bool LocalizationLidarProcess::LoadLidarExtrinsic(const std::string& file_path, TransformD* lidar_extrinsic) 
{
