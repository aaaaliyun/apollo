...
public:
        ...
        struct Leaf 
        {
                Leaf() : nr_points_(0), mean_(Eigen::Vector3d::Zero()), centroid(), cov_(Eigen::Matrix3d::Identity()), icov_(Eigen::Matrix3d::Zero()), evecs_(Eigen::Matrix3d::Identity()), evals_(Eigen::Vector3d::Zero()) {}
...
public:
        VoxelGridCovariance() : searchable_(true), min_points_per_voxel_(6), min_covar_eigvalue_mult_(0.01), leaves_(), voxel_centroids_(), voxel_centroidsleaf_indices_(), kdtree_() 
        {
        ...
        inline void SetMinPointPerVoxel(int min_points_per_voxel) 
        {
                ...
                {
                        PCL_WARN("%s, Covariance need 3 pts, set min_pt_per_vexel to 3", this->getClassName().c_str());
...
private:
        ...
        void ApplyFilter(PointCloudPtr output) 
        {
                ...
                {
                        PCL_WARN("[%s::ApplyFilter] leaf size is too small. Integer indices would overflow.", getClassName().c_str());
                        output->clear();
                        return;
                }
                        ...
                        for (size_t cp = 0; cp < input_->points.size(); ++cp) 
                        {
                                ...
                                else 
                                {
                                        ...
                                        if (rgba_index >= 0) 
                                        {
                                                ...
                                                const pcl::RGB& rgb = *reinterpret_cast<const pcl::RGB*>(reinterpret_cast<const char*>(&input_->points[cp]) + rgba_index);
                ...
                for (typename std::map<size_t, Leaf>::iterator it = leaves_.begin(); it != leaves_.end(); ++it) 
                {
                        ...
                        if (leaf.nr_points_ >= min_points_per_voxel_) 
                        {
                                ...
                                else 
                                {
                                        ...
                                        if (rgba_index >= 0) 
                                        {
                                                pcl::RGB& rgb = *reinterpret_cast<pcl::RGB*>(reinterpret_cast<char*>(&output->points.back()) + rgba_index);
