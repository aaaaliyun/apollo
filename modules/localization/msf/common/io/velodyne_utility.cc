              ...
              bool is_global) 
{
        ...
        LoadPcds(file_path, frame_index, pose, &velodyne_frame->pt3ds, &velodyne_frame->intensities, is_global);
}
                      ...
                      std::vector<double>* timestamps) 
{
        ...
        if (file) 
        {
                ...
                while (fscanf(file, "%u %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf\n", &index, &timestamp, &x, &y, &z, &qx, &qy, &qz, &qr, &std_x, &std_y, &std_z) == kSize) 
                {
                        Eigen::Translation3d trans(Eigen::Vector3d(x, y, z));
                        Eigen::Quaterniond quat(qr, qx, qy, qz);
                        poses->push_back(trans * quat);
