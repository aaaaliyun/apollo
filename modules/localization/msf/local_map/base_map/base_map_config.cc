...
bool BaseMapConfig::Load(const std::string file_path) 
{
        ...
        {
                AERROR << "[Fatal Error] Expect v" << map_version_ << " map, but found v" << map_version << " map.";
                return false;
        }
}
...
void BaseMapConfig::LoadXml(const boost::property_tree::ptree& config) 
{
        ...
        std::for_each(resolutions.begin(), resolutions.end(), [this](const boost::property_tree::ptree::value_type& v) 
        {
        ...
        std::for_each(datasets.begin(), datasets.end(), [this](const boost::property_tree::ptree::value_type& v) 
        {
