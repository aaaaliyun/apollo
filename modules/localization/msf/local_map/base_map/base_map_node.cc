...
void BaseMapNode::Init(const BaseMapConfig* map_config, const MapNodeIndex& index, bool create_map_cells) 
{
...
bool BaseMapNode::GetCoordinate(const Eigen::Vector2d& coordinate, unsigned int* x, unsigned int* y) const 
{
        ...
        if (off_x >= 0 && off_x < static_cast<int>(this->map_config_->map_node_size_x_) && off_y >= 0 && off_y < static_cast<int>(this->map_config_->map_node_size_y_)) 
        {
                *x = static_cast<unsigned int>(off_x);
                *y = static_cast<unsigned int>(off_y);
                return true;
...
Eigen::Vector2d BaseMapNode::GetLeftTopCorner(const BaseMapConfig& config, const MapNodeIndex& index) 
{
