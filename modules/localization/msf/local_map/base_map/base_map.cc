...
void BaseMap::PreloadMapNodes(std::set<MapNodeIndex>* map_ids) 
{
        ...
        while (itr != map_ids->end()) 
        {
                ...
                lock3.unlock();
                preload_futures.emplace_back(cyber::Async(&BaseMap::LoadMapNodeThreadSafety, this, *itr, false));
                ++itr;
        }
}
                             ...
                             unsigned int resolution_id, unsigned int zone_id) 
{
        ...
        pt_top_left[0] = location[0] - (static_cast<float>(this->map_config_->map_node_size_x_) * map_pixel_resolution / 2.0f);
        pt_top_left[1] = location[1] - (static_cast<float>(this->map_config_->map_node_size_y_) * map_pixel_resolution / 2.0f);
        pt_top_left[2] = 0;
        MapNodeIndex map_id = MapNodeIndex::GetMapNodeIndex(*(this->map_config_), pt_top_left, resolution_id, zone_id);
        ...
        pt_top_center[2] = 0;
        map_id = MapNodeIndex::GetMapNodeIndex(*(this->map_config_), pt_top_center, resolution_id, zone_id);
        map_ids.insert(map_id);

        /// top right
        Eigen::Vector3d pt_top_right;
        pt_top_right[0] = location[0] + (static_cast<float>(this->map_config_->map_node_size_x_) * map_pixel_resolution / 2.0);
        pt_top_right[1] = pt_top_left[1];
        pt_top_right[2] = 0;
        map_id = MapNodeIndex::GetMapNodeIndex(*(this->map_config_), pt_top_right, resolution_id, zone_id);
        map_ids.insert(map_id);
        ...
        pt_middle_left[2] = 0;
        map_id = MapNodeIndex::GetMapNodeIndex(*(this->map_config_), pt_middle_left, resolution_id, zone_id);
        map_ids.insert(map_id);
        ...
        pt_middle_right[2] = 0;
        map_id = MapNodeIndex::GetMapNodeIndex(*(this->map_config_), pt_middle_right, resolution_id, zone_id);
        map_ids.insert(map_id);

        /// bottom left
        Eigen::Vector3d pt_bottom_left;
        pt_bottom_left[0] = pt_top_left[0];
        pt_bottom_left[1] = location[1] + (static_cast<float>(this->map_config_->map_node_size_y_) * map_pixel_resolution / 2.0);
        pt_bottom_left[2] = 0;
        map_id = MapNodeIndex::GetMapNodeIndex(*(this->map_config_), pt_bottom_left, resolution_id, zone_id);
        map_ids.insert(map_id);
        ...
        map_id = MapNodeIndex::GetMapNodeIndex(*(this->map_config_), pt_bottom_center, resolution_id, zone_id);
        map_ids.insert(map_id);
        ...
        pt_bottom_right[2] = 0;
        map_id = MapNodeIndex::GetMapNodeIndex(*(this->map_config_), pt_bottom_right, resolution_id, zone_id);
        map_ids.insert(map_id);
                          ...
                          int filter_size_x, int filter_size_y) 
{
        ...
        /// top left
        Eigen::Vector3d pt_top_left;
        pt_top_left[0] = seed_pt3d[0] - (static_cast<float>(this->map_config_->map_node_size_x_) * map_pixel_resolution / 2.0f) - static_cast<float>(filter_size_x / 2) * map_pixel_resolution;
        pt_top_left[1] = seed_pt3d[1] - (static_cast<float>(this->map_config_->map_node_size_y_) * map_pixel_resolution / 2.0f) - static_cast<float>(filter_size_y / 2) * map_pixel_resolution;
        pt_top_left[2] = 0;
        MapNodeIndex map_id = MapNodeIndex::GetMapNodeIndex(*(this->map_config_), pt_top_left, resolution_id, zone_id);
        map_ids.insert(map_id);
        ...
        /// top right
        Eigen::Vector3d pt_top_right;
        pt_top_right[0] = seed_pt3d[0] + (static_cast<float>(this->map_config_->map_node_size_x_) * map_pixel_resolution / 2.0f) + static_cast<float>(filter_size_x / 2) * map_pixel_resolution;
        pt_top_right[1] = pt_top_left[1];
        pt_top_left[2] = 0;
        map_id = MapNodeIndex::GetMapNodeIndex(*(this->map_config_), pt_top_right, resolution_id, zone_id);
        map_ids.insert(map_id);
        ...
        Eigen::Vector3d pt_bottom_left;
        pt_bottom_left[0] = pt_top_left[0];
        pt_bottom_left[1] = seed_pt3d[1] + (static_cast<float>(this->map_config_->map_node_size_y_) * map_pixel_resolution / 2.0f) + static_cast<float>(filter_size_y / 2) * map_pixel_resolution;
        pt_bottom_left[2] = 0;
        map_id = MapNodeIndex::GetMapNodeIndex(*(this->map_config_), pt_bottom_left, resolution_id, zone_id);
        map_ids.insert(map_id);
