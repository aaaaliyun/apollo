...
LossyMapMatrix2D::LossyMapMatrix2D(const LossyMapMatrix2D& matrix) : BaseMapMatrix(matrix), map_cells_(NULL) 
{
...
uint16_t LossyMapMatrix2D::EncodeVar(const LossyMapCell2D& cell) const 
{
        ...
        var = std::sqrt(var);
        int intensity_var = static_cast<int>(static_cast<const float>(var_range_) / (var * static_cast<const float>(var_ratio_) + 1.0));
...
{
        float var = data;
        var = static_cast<float>((static_cast<const float>(var_range_) / var - 1.0) / static_cast<const float>(var_ratio_));
...
{
        unsigned int target_size = static_cast<unsigned int>(sizeof(unsigned int) * 2 + sizeof(float) * 4);
        // count, intensity, intensity_var, altitude_avg, altitude_ground
        target_size += static_cast<unsigned int>(rows_ * cols_ * (sizeof(unsigned char) + sizeof(unsigned char) + sizeof(uint16_t) + sizeof(uint16_t) + sizeof(uint16_t)));
        return target_size;
}
