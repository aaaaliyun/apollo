...
{
        unsigned int sz = static_cast<unsigned int>(sizeof(float) * 2 + sizeof(unsigned int) * 2 + sizeof(float) * 3 + sizeof(float) * 9);
        if (count_ >= minimum_points_threshold_) 
        {
...
void NdtMapCells::Reduce(NdtMapCells* cell, const NdtMapCells& cell_new) 
{
        ...
        for (auto it_new = cell_new.road_cell_indices_.begin(); it_new != cell_new.road_cell_indices_.end(); ++it_new) 
        {
                auto got_it = std::find(cell->road_cell_indices_.begin(), cell->road_cell_indices_.end(), *it_new);
        ...
        *intensity_img = cv::Mat(cv::Size(cols_, rows_), CV_8UC1);
        for (unsigned int y = 0; y < rows_; ++y) 
        {
                for (unsigned int x = 0; x < cols_; ++x) 
                {
                        ...
                        intensity_img->at<unsigned char>(y, x) = (unsigned char)(map3d_cells_[id].cells_[min_altitude_index].intensity_);
                }
        }
}
