...
inline void NdtMapSingleCell::AddSample(const float intensity, const float altitude,
...
inline void NdtMapSingleCell::CentroidEigenSolver(const Eigen::Matrix3f& centroid_cov) 
{
        ...
        if (count_ >= minimum_points_threshold_) 
        {
                ...
                if (centroid_icov_.maxCoeff() == std::numeric_limits<float>::infinity() || centroid_icov_.minCoeff() == -std::numeric_limits<float>::infinity()) 
                {
                        is_icov_available_ = 0;
                        return;
                }
