...
bool OfflineLocalVisualizer::Init(const std::string &map_folder, const std::string &map_visual_folder,
                                  ...
                                  const std::string &fusion_loc_file, const std::string &extrinsic_file) 
{
        ...
        success = visual_engine_.Init(map_folder_, map_visual_folder_, map_param, resolution_id_, zone_id_, velodyne_extrinsic_, LOC_INFO_NUM);
        if (!success) 
        {
    ...
    const std::vector<double> &pcd_timestamps) 
{
        ...
        PoseAndStdInterpolationByTime(poses, stds, timestamps, pcd_timestamps, &lidar_poses_, &lidar_stds_);
    ...
    const std::vector<double> &pcd_timestamps) 
{
        ...
        PoseAndStdInterpolationByTime(poses, stds, timestamps, pcd_timestamps, &fusion_poses_, &fusion_stds_);
        return true;
}
...
bool OfflineLocalVisualizer::GetZoneIdFromMapFolder(const std::string &map_folder, const unsigned int resolution_id, int *zone_id) 
{
        char buf[256];
