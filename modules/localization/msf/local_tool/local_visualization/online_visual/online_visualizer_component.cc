...
bool OnlineVisualizerComponent::InitConfig() 
{
        ...
        map_param.set(map_config.map_resolutions_, 
                      map_config.map_node_size_x_,
                      map_config.map_node_size_y_, 
                      map_config.map_range_.GetMinX(),
                      map_config.map_range_.GetMinY(),
                      map_config.map_range_.GetMaxX(),
                      map_config.map_range_.GetMaxY());

        success = VisualizationManager::GetInstance().Init(map_folder_, map_visual_folder_, velodyne_extrinsic, map_param);
        if (!success) 
        {
                return false;
        }
...
{
        // Lidar localization
        std::function<void(const std::shared_ptr<LocalizationEstimate> &)> lidar_local_call = std::bind(&OnlineVisualizerComponent::OnLidarLocalization, this, std::placeholders::_1);
        lidar_local_listener_ = this->node_->CreateReader<LocalizationEstimate>(lidar_local_topic_, lidar_local_call);

        // GNSS localization
        std::function<void(const std::shared_ptr<LocalizationEstimate> &)> gnss_local_call = std::bind(&OnlineVisualizerComponent::OnGNSSLocalization, this, std::placeholders::_1);
        gnss_local_listener_ = this->node_->CreateReader<LocalizationEstimate>(gnss_local_topic_, gnss_local_call);

        // Fusion localization
        std::function<void(const std::shared_ptr<LocalizationEstimate> &)> fusion_local_call = std::bind(&OnlineVisualizerComponent::OnFusionLocalization, this, std::placeholders::_1);
        fusion_local_listener_ = this->node_->CreateReader<LocalizationEstimate>(fusion_local_topic_, fusion_local_call);

        // Ndt Localization
        std::function<void(const std::shared_ptr<LocalizationEstimate> &)> ndt_local_call = std::bind(&OnlineVisualizerComponent::OnLidarLocalization, this, std::placeholders::_1);
        lidar_local_listener_ = this->node_->CreateReader<LocalizationEstimate>(ndt_local_topic_, ndt_local_call);
...
void OnlineVisualizerComponent::OnLidarLocalization(const std::shared_ptr<LocalizationEstimate> &msg) 
{
...
void OnlineVisualizerComponent::OnGNSSLocalization(const std::shared_ptr<LocalizationEstimate> &msg) 
{
...
void OnlineVisualizerComponent::OnFusionLocalization(const std::shared_ptr<LocalizationEstimate> &msg) 
{
        ...
        if (msg->height() > 1 && msg->width() > 1) 
        {
                for (unsigned int i = 0; i < msg->height(); ++i) 
                {
                        for (unsigned int j = 0; j < msg->width(); ++j) 
                        {
                                ...
                                if (!std::isnan(pt3d[0])) 
                                {
                                        unsigned char intensity = static_cast<unsigned char>(msg->point(i * msg->width() + j).intensity());
