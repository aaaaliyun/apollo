...
template <class MessageType>
void MessageBuffer<MessageType>::GetAllMessages(std::list<std::pair<double, MessageType>> *msg_list) 
{
        pthread_mutex_lock(&buffer_mutex_);
        msg_list->clear();
...
template <class MessageType>
bool IntepolationMessageBuffer<MessageType>::QueryMessage(const double timestamp, MessageType *msg, double timeout_s) 
{
...
template <class MessageType>
bool IntepolationMessageBuffer<MessageType>::WaitMessageBufferOk(const double timestamp, std::map<double, ListIterator> *msg_map,
                                ...
                                const VisualMapParam &map_param) 
{
        ...
        success = visual_engine_.Init(map_folder, map_visual_folder, map_param, resolution_id, zone_id, velodyne_extrinsic, LOC_INFO_NUM);
        if (!success) 
        {
...
bool VisualizationManager::Init(const VisualizationManagerParams &params) 
{
        ...
        return Init(params.map_folder, params.map_visual_folder, params.velodyne_extrinsic, params.map_param);
}
...
void VisualizationManager::DoVisualize() 
{
        while (!(stop_visualization_.load())) 
        {
                ...
                if (lidar_frame_buffer_.BufferSize() > 5) 
                {
                        ...
                        bool lidar_query_success = lidar_loc_info_buffer_.QueryMessage(lidar_frame.timestamp, &lidar_loc, 0);
                        // bool lidar_query_success = lidar_loc_info_buffer_.GetMessage(
                        //     lidar_frame.timestamp, lidar_loc);

                        bool fusion_query_success = fusion_loc_info_buffer_.QueryMessage(lidar_frame.timestamp, &fusion_loc, 0);
                        ...
                        if (lidar_query_success) 
                        {
                                ...
                                lidar_loc_info.set(trans, quat, lidar_std, "Lidar.", lidar_frame.timestamp, lidar_frame.frame_id);
                        }
                        ...
                        if (fusion_query_success) 
                        {
                                ...
                                fusion_loc_info.set(trans, quat, fusion_std, "Fusion.", lidar_frame.timestamp, lidar_frame.frame_id);
                        }
...
bool VisualizationManager::GetZoneIdFromMapFolder(const std::string &map_folder, const unsigned int resolution_id, int *zone_id) 
{
        char buf[256];
