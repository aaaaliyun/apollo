...
bool MapImageKey::operator<(const MapImageKey &key) const 
{
        ...
        return std::forward_as_tuple(level, zone_id, node_north_id, node_east_id) < std::forward_as_tuple(key.level, key.zone_id, key.node_north_id, key.node_east_id);
}
                               ...
                               const unsigned int loc_info_num) 
{
        ...
        cloud_img_mask_ = cv::Mat(cv::Size(map_param_.map_node_size_x, map_param_.map_node_size_y), CV_8UC1);
...
void VisualizationEngine::Visualize(const std::vector<LocalizatonInfo> &loc_infos, const ::apollo::common::EigenVector3dVec &cloud) 
{
        if (!is_init_) 
        {
                                     ...
                                     const std::string &map_visual_folder) 
{
        ...
        {
                AERROR << "image_visual_resolution_path: " << image_visual_resolution_path_ << " cannot be created.";
                return;
        }
        ...
        GenerateMutiResolutionImages(map_bin_path, static_cast<int>(image_resolution_path.length()), image_visual_resolution_path_);
}
...
void VisualizationEngine::Draw() 
{
        ...
        cv::resize(big_window_(cv::Rect(left_top_x, left_top_y, width, width)), image_window_, cv::Size(1024, 1024), 0, 0, CV_INTER_LINEAR);
                        ...
                        if (lt.x >= 0 && lt.y >= 0 && lt.x < 1024 * 3 && lt.y < 1024 * 3) 
                        {
                                if (is_draw_car_ && loc_info.is_has_attitude) 
                                {
                                        ...
                                        if (car_lt.x >= 0 && car_lt.y >= 0 && car_rb.x <= 1024 * 3 && car_rb.y <= 1024 * 3) 
                                        {
                                                ...
                                                rotated_mat.copyTo(big_window_(cv::Rect(car_lt.x, car_lt.y, rotated_mat.cols, rotated_mat.rows)), mat_mask);
                                        }
                                } 
                                else 
                                {
...
void VisualizationEngine::DrawLegend() 
{
        ...
        for (unsigned int i = 0; i < loc_info_num_; i++) 
        {
                ...
                cv::putText(image_window_, text, textOrg, fontFace, fontScale, cv::Scalar(255, 0, 0), thickness, 8);

                unsigned char b = color_table[i % 3][0];
                unsigned char g = color_table[i % 3][1];
                unsigned char r = color_table[i % 3][2];
                cv::circle(image_window_, cv::Point(755, (15 + textSize.height) * (i + 1) - textSize.height / 2), 8, cv::Scalar(b, g, r), 3);
        }
}
...
void VisualizationEngine::DrawTips() 
{
        ...
        // draw tips.
        char info[256];
        snprintf(info, sizeof(info), "e: zoom out; q: zoom in; m: max scale; n: origin scale; w: up; s: "
                 "down; a: move left; d: move right; r: move center");
        text = info;
        textSize = cv::getTextSize(text, fontFace, fontScale, thickness, &baseline);
        cv::Point textOrg(5, 5 + textSize.height);
        cv::putText(tips_window_, text, textOrg, fontFace, fontScale, cv::Scalar(255, 255, 255), thickness, 8);

        snprintf(info, sizeof(info), "f: follow car/free view; p: play/pause; c: change current loc; 1: "
...
void VisualizationEngine::GenerateMutiResolutionImages(const std::vector<std::string> &src_files, const int base_path_length, const std::string &dst_folder) 
{
        int x_min = INT_MAX;
        int x_max = -1;
        int y_min = INT_MAX;
        ...
        {
                std::string p = src_files[i].substr(base_path_length, src_files[i].length() - 4 - base_path_length);
                p = dst_folder + p;
        ...
        std::string image_visual_path_dst = src_files[0].substr(base_path_length, src_files[0].length() - 22 - base_path_length);
                        ...
                        for (pt_y = y_min; pt_y < y_max; pt_y += nstep) 
                        {
                                ...
                                for (int i = 0; i < 2; i++) 
                                {
                                        for (int j = 0; j < 2; j++) 
                                        {
                                                ...
                                                image_visual_path_dst.c_str(), pt_y + i * step, pt_x + j * step, lvl - 1);
                                ...
                                if (flag) 
                                {
                                        snprintf(ss, sizeof(ss), "%s/%08d/%08d_%d.png", image_visual_path_dst.c_str(), pt_y, pt_x, lvl);
        ...
        outf << level << std::endl;
        outf << image_visual_path_dst.substr(dst_folder.length(), image_visual_path_dst.length() - dst_folder.length() - 3) << std::endl;
        outf.close();
}
...
void VisualizationEngine::CloudToMat(const Eigen::Affine3d &cur_pose, const Eigen::Affine3d &velodyne_extrinsic, const ::apollo::common::EigenVector3dVec &cloud, cv::Mat *cloud_img, cv::Mat *cloud_img_mask) 
{
        ...
        cloud_img_lt_coord_[0] = cen[0] - map_param_.map_resolutions[resolution_id_] * (static_cast<float>(img_width) / 2.0f);
        cloud_img_lt_coord_[1] = cen[1] - map_param_.map_resolutions[resolution_id_] * (static_cast<float>(img_height) / 2.0f);
        ...
        for (unsigned int i = 0; i < cloud.size(); i++) 
        {
                ...
                int col = static_cast<int>((pt_global[0] - cloud_img_lt_coord_[0]) / map_param_.map_resolutions[resolution_id_]);
                int row = static_cast<int>((pt_global[1] - cloud_img_lt_coord_[1]) / map_param_.map_resolutions[resolution_id_]);
                if (col < 0 || row < 0 || col >= static_cast<int>(map_param_.map_node_size_x) || row >= static_cast<int>(map_param_.map_node_size_y)) 
                {
                        continue;
                }
                                          ...
                                          MapImageKey *key) 
{
        ...
        int n = static_cast<int>((coord[0] - map_param_.map_min_x) / (static_cast<float>(map_param_.map_node_size_x) * map_param_.map_resolutions[resolution_id_]));
        int m = static_cast<int>((coord[1] - map_param_.map_min_y) / (static_cast<float>(map_param_.map_node_size_y) * map_param_.map_resolutions[resolution_id_]));
        int max_n = static_cast<int>((map_param_.map_max_x - map_param_.map_min_x) / (static_cast<float>(map_param_.map_node_size_x) * map_param_.map_resolutions[resolution_id_]));
        int max_m = static_cast<unsigned int>((map_param_.map_max_y - map_param_.map_min_y) / (static_cast<float>(map_param_.map_node_size_y) * map_param_.map_resolutions[resolution_id_]));
...
{
        cv::Point p;
        p.x = static_cast<int>((coord[0] - map_param_.map_min_x) / map_param_.map_resolutions[resolution_id]);
        p.y = static_cast<int>((coord[1] - map_param_.map_min_y) / map_param_.map_resolutions[resolution_id]);
...
bool VisualizationEngine::LoadImageToCache(const MapImageKey &key) 
{
        ...
        {
                char path[1024];
                snprintf(path, sizeof(path), "%s/%02d/%08d/%08d_%d.png", image_visual_leaf_path_.c_str(), key.zone_id, key.node_north_id, key.node_east_id, key.level);
                                    ...
                                    double angle) 
{
        ...
        in_img.copyTo(mat_tem(cv::Rect(width / 2 - in_img.cols / 2, width / 2 - in_img.rows / 2, in_img.cols, in_img.rows)));
