...
int main(int argc, char **argv) 
{
        ...
        ("cloud_topic", boost::program_options::value<std::string>()->default_value("/apollo/sensor/velodyne64/compensator/PointCloud2"), "provide point cloud2 topic")
        ("gnss_loc_topic", boost::program_options::value<std::string>()->default_value("/apollo/localization/measure_gnss"), "provide gnss localization topic")
        ("lidar_loc_topic", boost::program_options::value<std::string>()->default_value("/apollo/localization/measure_lidar"), "provide lidar localization topic")
        ("fusion_loc_topic", boost::program_options::value<std::string>()->default_value("/apollo/localization/pose"), "provide fusion localization topic")
        ("odometry_loc_topic", boost::program_options::value<std::string>()->default_value("/apollo/sensor/gnss/odometry"), "provide odometry localization topic");
        ...
        if (boost_args.count("help") || !boost_args.count("bag_file") || !boost_args.count("out_folder")) 
        {
                AERROR << boost_desc;
                return 0;
        }
