...
MapNodeIndex GetMapIndexFromMapFolder(const std::string& map_folder) 
{
        MapNodeIndex index;
        char buf[100];
        sscanf(map_folder.c_str(), "/%03u/%05s/%02d/%08u/%08u", &index.resolution_id_, buf, &index.zone_id_, &index.m_, &index.n_);
        std::string zone = buf;
        if (zone == "south") 
        {
...
int main(int argc, char** argv) 
{
        ...
        ("srcdir", boost::program_options::value<std::string>(), "provide the data base dir")
        ("dstdir", boost::program_options::value<std::string>(), "provide the lossy map destination dir");
        ...
        for (; itr != buf.end(); ++itr, ++index) 
        {
                ...
                PyramidMapMatrix& lossless_matrix = static_cast<PyramidMapMatrix&>(lossless_node->GetMapCellMatrix());
