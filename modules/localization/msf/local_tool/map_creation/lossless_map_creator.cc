                      ...
                      boost::program_options::variables_map* vm) 
{
        ...
        ("use_plane_inliers_only", boost::program_options::value<bool>()->required(), "use plane inliers only")
        // ("use_plane_fitting_ransac",
        // boost::program_options::value<bool>()->required(),
        //  "use plane fitting ransac")
        ("pcd_folders", boost::program_options::value<std::vector<std::string>>()->multitoken()->composing()->required(), "pcd folders(repeated)")
        ("pose_files", boost::program_options::value<std::vector<std::string>>()->multitoken()->composing()->required(), "pose files(repeated)")
        ("map_folder", boost::program_options::value<std::string>()->required(), "map folder")
        ("zone_id", boost::program_options::value<int>()->required(), "zone id")
        ("coordinate_type", boost::program_options::value<std::string>()->required(), "coordinate type: UTM or LTM")
        ("map_resolution_type", boost::program_options::value<std::string>()->required(), "map resolution type: single or multi")
        ("resolution", boost::program_options::value<float>()->default_value(0.125), "optional: resolution for single resolution generation, default: 0.125");
...
int main(int argc, char** argv) 
{
        ...
        else 
        {
                ...
                loss_less_config.map_range_ = apollo::localization::msf::Rect2D<double>(-1638400.0, -1638400.0, 1638400.0, 1638400.0);
        }
        ...
        if (file) 
        {
                ...
                fprintf(file, "Map node size: %d x %d\n", loss_less_config.map_node_size_x_, loss_less_config.map_node_size_y_);
                fprintf(file, "Map row x col: \n");
                for (size_t i = 0; i < loss_less_config.map_resolutions_.size(); ++i) 
                {
                        fprintf(file, "%u x %u, ", MapNodeIndex::GetMapIndexRangeNorth(loss_less_config, static_cast<unsigned int>(i)),
        ...
        for (unsigned int trial = 0; trial < num_trials; ++trial) 
        {
                for (unsigned int frame_idx = 0; frame_idx < ieout_poses[trial].size(); ++frame_idx) 
                {
                        ...
                        for (size_t i = 0; i < velodyne_frame.pt3ds.size(); ++i) 
                        {
                                ...
                                MapNodeIndex map_node_index = MapNodeIndex::GetMapNodeIndex(loss_less_config, pt3d_global, resolution_id, zone_id);
        ...
        for (unsigned int trial = 0; trial < num_trials; ++trial) 
        {
                for (unsigned int i = 0; i < ieout_poses[trial].size(); ++i) 
                {
                        ...
                        unsigned int row = 0;
                        unsigned int col = 0;
                        MapNodeIndex map_node_index = MapNodeIndex::GetMapNodeIndex(loss_less_config, pt3d, resolution_id, zone_id);
                        PyramidMapNode* map_node = dynamic_cast<PyramidMapNode*>(map.GetMapNodeSafe(map_node_index));
