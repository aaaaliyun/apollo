...
void PosesInterpolation::DoInterpolation() 
{
        ...
        velodyne::LoadPosesAndStds(input_poses_path_, &input_poses_, &input_stds, &input_poses_timestamps_);

        // Load pcd timestamp
        LoadPCDTimestamp();

        // Interpolation
        PoseInterpolationByTime(input_poses_, input_poses_timestamps_, ref_timestamps_, ref_ids_, &out_indexes_, &out_timestamps_, &out_poses_);
...
void PosesInterpolation::PoseInterpolationByTime(const std::vector<Eigen::Affine3d> &in_poses, const std::vector<double> &in_timestamps, 
                                                 const std::vector<double> &ref_timestamps, const std::vector<unsigned int> &ref_indexes, 
