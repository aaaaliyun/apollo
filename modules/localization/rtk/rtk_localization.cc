...
{
        double time_delay = last_received_timestamp_sec_ ? common::time::Clock::NowInSeconds() - last_received_timestamp_sec_ : last_received_timestamp_sec_;
...
void RTKLocalization::RunWatchDog(double gps_timestamp) 
{
        ...
        double gps_service_delay = common::time::Clock::NowInSeconds() - service_started_time;
        ...
        lock.unlock();
        double imu_delay_sec = common::time::Clock::NowInSeconds() - imu_msg.header().timestamp_sec();
        ...
        if (imu_delay_cycle_cnt > report_threshold_err_num_ && static_cast<int>(gps_service_delay) > service_delay_threshold) 
        {
                msg_delay = true;
                std::stringstream ss;
                ss << "Raw IMU Message Delay. IMU message is " << imu_delay_cycle_cnt << " cycle " << imu_delay_sec << " sec behind current time.";
                monitor_logger_.ERROR(ss.str());
        }

        // to prevent it from beeping continuously
        if (msg_delay && (last_reported_timestamp_sec_ < 1. || common::time::Clock::NowInSeconds() > last_reported_timestamp_sec_ + 1.)) 
        {
...
void RTKLocalization::ComposeLocalizationMsg(const localization::Gps &gps_msg, const localization::CorrectedImu &imu_msg, LocalizationEstimate *localization) 
{
        ...
        if (gps_msg.has_localization()) 
        {
                ...
                if (pose.has_orientation()) 
                {
                        ...
                        double heading = common::math::QuaternionToHeading(pose.orientation().qw(), pose.orientation().qx(),
        ...
        if (imu_msg.has_imu()) 
        {
                ...
                if (imu.has_angular_velocity()) 
                {
                        if (localization->pose().has_orientation()) 
                        {
                                // angular_velocity:
                                // convert from vehicle reference to map reference
                                Vector3d orig(imu.angular_velocity().x(), imu.angular_velocity().y(), imu.angular_velocity().z());
                                Vector3d vec = common::math::QuaternionRotate(localization->pose().orientation(), orig);
