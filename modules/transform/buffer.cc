...
int Buffer::Init() 
{
        const std::string node_name = absl::StrCat("transform_listener_", cyber::Time::Now().ToNanosecond());
        ...
        message_subscriber_tf_static_ = node_->CreateReader<TransformStampeds>(attr_static, [&](const std::shared_ptr<TransformStampeds>& msg_evt) 
        {
...
void Buffer::SubscriptionCallbackImpl(const std::shared_ptr<const TransformStampeds>& msg_evt, bool is_static) 
{
...
void Buffer::TF2MsgToCyber(const geometry_msgs::TransformStamped& tf2_trans_stamped, TransformStamped& trans_stamped) const 
{
        // header
        trans_stamped.mutable_header()->set_timestamp_sec(static_cast<double>(tf2_trans_stamped.header.stamp) / 1e9);
        ...
        // rotation
        trans_stamped.mutable_transform()->mutable_rotation()->set_qx(tf2_trans_stamped.transform.rotation.x);
        trans_stamped.mutable_transform()->mutable_rotation()->set_qy(tf2_trans_stamped.transform.rotation.y);
        trans_stamped.mutable_transform()->mutable_rotation()->set_qz(tf2_trans_stamped.transform.rotation.z);
        trans_stamped.mutable_transform()->mutable_rotation()->set_qw(tf2_trans_stamped.transform.rotation.w);
}
                                         ...
                                         const float timeout_second) const 
{
        ...
        geometry_msgs::TransformStamped tf2_trans_stamped = lookupTransform(target_frame, source_frame, tf2_time);
...
TransformStamped Buffer::lookupTransform(const std::string& target_frame,
                                         ...
                                         const float timeout_second) const 
{
        geometry_msgs::TransformStamped tf2_trans_stamped = lookupTransform(target_frame, target_time.ToNanosecond(), source_frame, source_time.ToNanosecond(), fixed_frame);
        TransformStamped trans_stamped;
        TF2MsgToCyber(tf2_trans_stamped, trans_stamped);
        return trans_stamped;
}
                          ...
                          std::string* errstr) const 
{
        ...
        while (cyber::Time::Now().ToNanosecond() < start_time + timeout_ns && !canTransform(target_frame, source_frame, time.ToNanosecond(), errstr) && !cyber::IsShutdown()) 
        {
                          ...
                          std::string* errstr) const 
{
        ...
        while (cyber::Time::Now().ToNanosecond() < start_time + timeout_ns &&
               !canTransform(target_frame, target_time.ToNanosecond(), source_frame, source_time.ToNanosecond(), fixed_frame) &&
               !cyber::IsShutdown()) 
        {  
                // Make sure we haven't been stopped
                std::this_thread::sleep_for(std::chrono::milliseconds(3));
        }
        bool retval = canTransform(target_frame, target_time.ToNanosecond(), source_frame, source_time.ToNanosecond(), fixed_frame, errstr);
        return retval;
}
