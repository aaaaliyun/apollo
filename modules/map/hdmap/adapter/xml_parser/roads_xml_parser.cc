...
Status RoadsXmlParser::Parse(const tinyxml2::XMLElement& xml_node, std::vector<RoadInternal>* roads) 
{
        CHECK_NOTNULL(roads);
...
void RoadsXmlParser::Parse_road_objects(const tinyxml2::XMLElement& xml_node, RoadInternal* road_info) 
{
...
void RoadsXmlParser::Parse_road_signals(const tinyxml2::XMLElement& xml_node, RoadInternal* road_info) 
{
        CHECK_NOTNULL(road_info);
