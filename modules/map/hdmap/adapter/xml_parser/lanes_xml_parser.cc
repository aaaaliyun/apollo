                             ...
                             std::vector<RoadSectionInternal>* sections) 
{
        ...
        while (sub_node) 
        {
                ...
                RETURN_IF_ERROR(ParseSectionBoundary(*sub_node, section_internal.section.mutable_boundary()->mutable_outer_polygon()));
                                        ...
                                        std::vector<LaneInternal>* lanes) 
{
        ...
        *(reference_lane_internal.lane.mutable_left_boundary()) = reference_lane_internal.lane.right_boundary();
        if (lanes->size() > 0) 
        {
                PbLane& left_neighbor_lane = lanes->back().lane;
...
Status LanesXmlParser::ParseSingleSideRoadSampleAssociates(const tinyxml2::XMLElement& xml_node, bool bleft, PbLane* lane) 
{
        CHECK_NOTNULL(lane);

        auto sub_node = xml_node.FirstChildElement(bleft ? "leftRoadSampleAssociations" : "rightRoadSampleAssociations");
        if (sub_node == nullptr) 
        {
...
Status LanesXmlParser::ParseLeftRoadSampleAssociates(const tinyxml2::XMLElement& xml_node, PbLane* lane) 
{
        CHECK_NOTNULL(lane);
...
Status LanesXmlParser::ParseRightRoadSampleAssociates(const tinyxml2::XMLElement& xml_node, PbLane* lane) 
{
        CHECK_NOTNULL(lane);
...
Status LanesXmlParser::ParseRoadSampleAssociates(const tinyxml2::XMLElement& xml_node, PbLane* lane) 
{
        CHECK_NOTNULL(lane);
...
Status LanesXmlParser::ParseObjectOverlapGroup(const tinyxml2::XMLElement& xml_node, std::vector<OverlapWithLane>* object_overlaps) 
{
        ...
        if (object_overlap) 
        {
                ...
                while (sub_node) 
                {
                        ...
                        {
                                UtilXmlParser::QueryStringAttribute(*sub_node, "regionOverlapId", &overlap_with_lane.region_overlap_id);
                        }
...
Status LanesXmlParser::ParseSignalOverlapGroup(const tinyxml2::XMLElement& xml_node, std::vector<OverlapWithLane>* signal_overlaps) 
{
...
Status LanesXmlParser::ParseJunctionOverlapGroup(const tinyxml2::XMLElement& xml_node, std::vector<OverlapWithLane>* junction_overlaps) 
{
...
Status LanesXmlParser::ParseLaneOverlapGroup(const tinyxml2::XMLElement& xml_node, std::vector<OverlapWithLane>* lane_overlaps) 
{
...
Status LanesXmlParser::ParseLaneBorderMark(const tinyxml2::XMLElement& xml_node, PbLaneBoundaryTypeType* boundary_type) 
{
        CHECK_NOTNULL(boundary_type);
...
Status LanesXmlParser::ParseRegionOverlap(const tinyxml2::XMLElement& xml_node, std::vector<PbRegionOverlap>* region_overlaps) 
{
        ...
        while (region_overlap_node != nullptr) 
        {
                ...
                int checker = UtilXmlParser::QueryStringAttribute(*region_overlap_node, "id", &region_overlap_id);
                if (checker != tinyxml2::XML_SUCCESS) 
                {
