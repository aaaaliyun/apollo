...
void ProtoOrganizer::GetLaneObjectOverlapElements(const std::string& lane_id, const std::vector<OverlapWithLane>& overlap_with_lanes) 
{
        for (auto& overlap_object : overlap_with_lanes) 
        {
                ...
                {
                        object_overlap->mutable_lane_overlap_info()->mutable_region_overlap_id()->set_id(overlap_object.region_overlap_id);
                }
                ...
                if (proto_data_.pb_crosswalks.count(object_id) > 0) 
                {
                        ...
                        if (!overlap_object.region_overlap_id.empty()) 
                        {
                                object_overlap->mutable_crosswalk_overlap_info()->mutable_region_overlap_id()->set_id(overlap_object.region_overlap_id);
                        }
...
void ProtoOrganizer::GetLaneSignalOverlapElements(const std::string& lane_id, const std::vector<OverlapWithLane>& overlap_with_lanes) 
{
...
void ProtoOrganizer::GetLaneJunctionOverlapElements(const std::string& lane_id, const std::vector<OverlapWithLane>& overlap_with_lanes) 
{
...
void ProtoOrganizer::GetJunctionObjectOverlapElements(const std::vector<JunctionInternal>& junctions) 
{
        for (auto& junction_internal : junctions) 
        {
...
void ProtoOrganizer::GetOverlapElements(const std::vector<RoadInternal>& roads, const std::vector<JunctionInternal>& junctions) 
{
        std::unordered_map<std::pair<std::string, std::string>, OverlapWithLane, PairHash> lane_lane_overlaps;
        // overlap
        for (auto& road_internal : roads) 
        {
