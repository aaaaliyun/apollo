...
apollo::common::math::Polygon2d ConvertToPolygon2d(const Polygon &polygon) 
{
        ...
        RemoveDuplicates(&points);
        while (points.size() >= 2 && points[0].DistanceTo(points.back()) <= apollo::common::math::kMathEpsilon) 
        {
                points.pop_back();
        }
...
void SegmentsFromCurve(const Curve &curve, std::vector<apollo::common::math::LineSegment2d> *segments) 
{
...
double LaneInfo::Heading(const double s) const 
{
        ...
        return common::math::slerp(headings_[index - 1], accumulated_s_[index - 1], headings_[index], accumulated_s_[index], s);
}
...
double LaneInfo::Curvature(const double s) const 
{
        ...
        return (headings_[index] - headings_[index - 1]) / (accumulated_s_[index] - accumulated_s_[index - 1] + kEpsilon);
}
...
void LaneInfo::CreateKDTree() 
{
        ...
        for (size_t id = 0; id < segments_.size(); ++id) 
        {
                ...
                segment_box_list_.emplace_back(apollo::common::math::AABox2d(segment.start(), segment.end()), this, &segment, id);
        }
...
YieldSignInfo::YieldSignInfo(const YieldSign &yield_sign) : yield_sign_(yield_sign) 
{
        Init();
}
...
SpeedBumpInfo::SpeedBumpInfo(const SpeedBump &speed_bump) : speed_bump_(speed_bump) 
{
        Init();
}
...
ParkingSpaceInfo::ParkingSpaceInfo(const ParkingSpace &parking_space) : parking_space_(parking_space) 
{
        Init();
}
...
PNCJunctionInfo::PNCJunctionInfo(const PNCJunction &pnc_junction) : junction_(pnc_junction) 
{
        Init();
}
