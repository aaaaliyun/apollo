...
int HDMapImpl::GetJunctions(const PointENU& point, double distance, std::vector<JunctionInfoConstPtr>* junctions) const 
{
...
int HDMapImpl::GetCrosswalks(const PointENU& point, double distance, std::vector<CrosswalkInfoConstPtr>* crosswalks) const 
{
        return GetCrosswalks({point.x(), point.y()}, distance, crosswalks);
}

int HDMapImpl::GetCrosswalks(const Vec2d& point, double distance, std::vector<CrosswalkInfoConstPtr>* crosswalks) const 
{
...
int HDMapImpl::GetStopSigns(const PointENU& point, double distance, std::vector<StopSignInfoConstPtr>* stop_signs) const 
{
...
int HDMapImpl::GetYieldSigns(const PointENU& point, double distance, std::vector<YieldSignInfoConstPtr>* yield_signs) const 
{
        return GetYieldSigns({point.x(), point.y()}, distance, yield_signs);
}

int HDMapImpl::GetYieldSigns(const Vec2d& point, double distance, std::vector<YieldSignInfoConstPtr>* yield_signs) const 
{
...
int HDMapImpl::GetClearAreas(const PointENU& point, double distance, std::vector<ClearAreaInfoConstPtr>* clear_areas) const 
{
        return GetClearAreas({point.x(), point.y()}, distance, clear_areas);
}

int HDMapImpl::GetClearAreas(const Vec2d& point, double distance, std::vector<ClearAreaInfoConstPtr>* clear_areas) const 
{
...
int HDMapImpl::GetSpeedBumps(const PointENU& point, double distance, std::vector<SpeedBumpInfoConstPtr>* speed_bumps) const 
{
        return GetSpeedBumps({point.x(), point.y()}, distance, speed_bumps);
}

int HDMapImpl::GetSpeedBumps(const Vec2d& point, double distance, std::vector<SpeedBumpInfoConstPtr>* speed_bumps) const 
{
...
int HDMapImpl::GetParkingSpaces(const PointENU& point, double distance, std::vector<ParkingSpaceInfoConstPtr>* parking_spaces) const 
{
        return GetParkingSpaces({point.x(), point.y()}, distance, parking_spaces);
}

int HDMapImpl::GetParkingSpaces(const Vec2d& point, double distance, std::vector<ParkingSpaceInfoConstPtr>* parking_spaces) const 
{
...
int HDMapImpl::GetPNCJunctions(const apollo::common::PointENU& point, double distance, std::vector<PNCJunctionInfoConstPtr>* pnc_junctions) const 
{
        return GetPNCJunctions({point.x(), point.y()}, distance, pnc_junctions);
}

int HDMapImpl::GetPNCJunctions(const apollo::common::math::Vec2d& point, double distance, std::vector<PNCJunctionInfoConstPtr>* pnc_junctions) const 
{
...
int HDMapImpl::GetNearestLane(const Vec2d& point, LaneInfoConstPtr* nearest_lane, double* nearest_s, double* nearest_l) const 
{
...
int HDMapImpl::GetRoadBoundaries(const PointENU& point, double radius, std::vector<RoadRoiPtr>* road_boundaries, std::vector<JunctionInfoConstPtr>* junctions) const 
{
        ...
        for (const auto& lane_ptr : lanes) 
        {
                ...
                for (const auto& overlap_id : lane_ptr->lane().overlap_id()) 
                {
                        ...
                        for (int i = 0; i < overlap_ptr->overlap().object_size(); ++i) 
                        {
                                ...
                                {
                                        ParkingSpaceInfoConstPtr parkingspace_ptr = GetParkingSpaceById(overlap_ptr->overlap().object(i).id());
        ...
        while (lane_ptr != nullptr) 
        {
                ...
                for (const auto& overlap_id : lane_ptr->lane().overlap_id()) 
                {
                        ...
                        for (int i = 0; i < overlap_ptr->overlap().object_size(); ++i) 
                        {
                                if (overlap_ptr->overlap().object(i).id().id() == lane_ptr->id().id()) 
                                {
                                        lane_overlap_offset_s = overlap_ptr->overlap().object(i).lane_overlap_info().start_s() - s_start;
                                        continue;
                                }
                                ...
                                else if (lane_overlap_offset_s < (signal_min_dist + 0.1) && lane_overlap_offset_s > (signal_min_dist - 0.1)) 
                                {
                                        min_dist_signal_ptr.push_back(signal_ptr);
                                }
                        }
                           ...
                           Map* local_map) const 
{
        ...
        for (auto& lane_ptr : lanes) 
        {
                ...
                std::copy(lane_ptr->lane().overlap_id().begin(), lane_ptr->lane().overlap_id().end(), std::back_inserter(overlap_ids));
        ...
        for (auto& crosswalk_ptr : crosswalks) 
        {
                ...
                std::copy(crosswalk_ptr->crosswalk().overlap_id().begin(), crosswalk_ptr->crosswalk().overlap_id().end(), std::back_inserter(overlap_ids));
        ...
        for (auto& junction_ptr : junctions) 
        {
                ...
                std::copy(junction_ptr->junction().overlap_id().begin(), junction_ptr->junction().overlap_id().end(), std::back_inserter(overlap_ids));
        ...
        for (auto& signal_ptr : signals) 
        {
                ...
                std::copy(signal_ptr->signal().overlap_id().begin(), signal_ptr->signal().overlap_id().end(), std::back_inserter(overlap_ids));
        ...
        for (auto& stop_sign_ptr : stop_signs) 
        {
                ...
                std::copy(stop_sign_ptr->stop_sign().overlap_id().begin(), stop_sign_ptr->stop_sign().overlap_id().end(), std::back_inserter(overlap_ids));
                *local_map->add_stop_sign() = stop_sign_ptr->stop_sign();
        }

        for (auto& yield_sign_ptr : yield_signs) 
        {
                std::copy(yield_sign_ptr->yield_sign().overlap_id().begin(), yield_sign_ptr->yield_sign().overlap_id().end(), std::back_inserter(overlap_ids));
        ...
        for (auto& clear_area_ptr : clear_areas) 
        {
                ...
                std::copy(clear_area_ptr->clear_area().overlap_id().begin(), clear_area_ptr->clear_area().overlap_id().end(), std::back_inserter(overlap_ids));
        ...
        for (auto& speed_bump_ptr : speed_bumps) 
        {
                ...
                std::copy(speed_bump_ptr->speed_bump().overlap_id().begin(), speed_bump_ptr->speed_bump().overlap_id().end(), std::back_inserter(overlap_ids));
        ...
        for (auto& parking_space_ptr : parking_spaces) 
        {
                ...
                std::copy(parking_space_ptr->parking_space().overlap_id().begin(), parking_space_ptr->parking_space().overlap_id().end(), std::back_inserter(overlap_ids));
...
int HDMapImpl::SearchObjects(const Vec2d& center, const double radius, const KDTree& kdtree, std::vector<std::string>* const results) 
{
