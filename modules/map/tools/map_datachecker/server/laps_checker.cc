...
LapsChecker::LapsChecker(const std::vector<FramePose> &poses, int laps_to_check, std::shared_ptr<JSonConf> sp_conf) : poses_(poses), sp_conf_(sp_conf) 
{
...
int LapsChecker::GetPassedGrid(int pose_index, std::vector<int> *sp_grid_x, std::vector<int> *sp_grid_y) 
{
        if (pose_index <= 0) 
        {
                return 0;
        ...
        if (std::abs(x - last_x) < 1e-6) 
        {  
                // current trace is vertical
