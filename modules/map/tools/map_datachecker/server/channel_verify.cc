...
ChannelVerify::ChannelVerify(std::shared_ptr<JSonConf> sp_conf) : sp_conf_(sp_conf) 
{
        Reset();
}
...
std::vector<std::string> ChannelVerify::GetRecordsPath(const std::string& record_dir_or_record_full_path) const 
{
...
std::shared_ptr<CyberRecordInfo> ChannelVerify::GetRecordInfo(const std::string& record_path) const 
{
        ...
        std::shared_ptr<apollo::cyber::record::RecordReader> sp_reader = std::make_shared<apollo::cyber::record::RecordReader>(record_path);
        ...
        sp_record_info->duration = static_cast<double>((sp_viewer->end_time() - sp_viewer->begin_time())) / 1e9;
