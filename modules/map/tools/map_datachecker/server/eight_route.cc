...
bool EightRoute::IsEightRoutePose(const std::vector<FramePose>& poses, int pose_index) 
{
        ...
        double yaw = GetYaw(poses[pose_index - 1].tx, poses[pose_index - 1].ty, poses[pose_index].tx, poses[pose_index].ty);
...
{
        if (sp_good_pose_info_ == nullptr || sp_good_pose_info_->start_time < 0 || sp_good_pose_info_->end_time < 0) 
        {
                return 0.0;
        }
...
double EightRoute::GetEightRouteProgress(const std::vector<FramePose>& poses) 
{
        ...
        last_yaw_ = GetYaw(poses[start_index].tx, poses[start_index].ty, poses[start_index + 1].tx, poses[start_index + 1].ty);
