...
grpc::Status LoopsVerifyAgent::ProcessGrpcRequest(grpc::ServerContext *context, LoopsVerifyRequest *request, LoopsVerifyResponse *response) 
{
                                   ...
                                   LoopsVerifyResponse *response) 
{
        ...
        if (std::abs(1.0 - progress) < 1e-8) 
        {
                ...
                if (data_type == DataType::MAP_CHECKOUT) 
                {
                        ...
                        {
                                loop_result->set_loop_num(lap - sp_conf_->laps_number >= 0 ? static_cast<double>(lap - sp_conf_->laps_number) : 0.0);
                        }
                }
        }
}
...
double LoopsVerifyAgent::GetRangeIndex(std::shared_ptr<std::vector<std::pair<double, double>>> sp_range,
...
int LoopsVerifyAgent::GetPosesToCheck(std::shared_ptr<std::vector<std::pair<double, double>>> sp_range, std::vector<FramePose> *sp_poses) 
{
...
int LoopsVerifyAgent::DoStartVerify(std::shared_ptr<std::vector<std::pair<double, double>>> sp_range, double loops_to_check) 
{
        clock_t start = clock();
        std::vector<FramePose> all_poses;
        GetPosesToCheck(sp_range, &all_poses);

        sp_laps_checker_ = std::shared_ptr<LapsChecker>(new LapsChecker(all_poses, static_cast<int>(loops_to_check), sp_conf_));
