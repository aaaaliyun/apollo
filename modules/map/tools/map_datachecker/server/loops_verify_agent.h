...
private:
        ...
        std::shared_ptr<std::vector<std::pair<double, double>>> get_verify_range(LoopsVerifyRequest *request);
        size_t GetLoopsToCheck(LoopsVerifyRequest *request);
        int GetPosesToCheck(std::shared_ptr<std::vector<std::pair<double, double>>> sp_range, std::vector<FramePose> *poses);
        int DoStartVerify(std::shared_ptr<std::vector<std::pair<double, double>>> sp_range, double loops_to_check);
        double GetRangeIndex(std::shared_ptr<std::vector<std::pair<double, double>>> sp_range, std::vector<bool> *range_index, std::shared_ptr<std::vector<FramePose>> sp_vec_poses);
