...
LoopsChecker::LoopsChecker(const std::string& time_flag_file) : time_flag_file_(time_flag_file) 
{
        YAML::Node node = YAML::LoadFile(FLAGS_client_conf_yaml);
        std::string server_addr = node["grpc_host_port"]["grpc_host"].as<std::string>() + ":" + node["grpc_host_port"]["grpc_port"].as<std::string>();
        check_period_ = node["loops_check"]["check_period"].as<int>();
        service_stub_ = CollectionCheckerService::NewStub(grpc::CreateChannel(server_addr, grpc::InsecureChannelCredentials()));
}
...
std::vector<std::pair<double, double>> LoopsChecker::GetTimeRanges() 
{
        ...
        for (size_t i = 0; i < record_count; i += 2) 
        {
                ...
                if (split_str[1] != "start") 
                {
                        ...
                        fprintf(USER_STREAM, "The data collect command start and stop should be appear in pairs\n");
                        return empty;
                }
