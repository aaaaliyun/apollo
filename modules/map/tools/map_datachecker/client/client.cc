...
Client::Client() 
{
        ...
        channel_checker_stop_flag_file_ = bin_path + "/" + node["channel_check"]["stop_flag_file"].as<std::string>();
...
int Client::StaticAlignStage() 
{
        ...
        if ("start" == cmd) 
        {
                ...
                else 
                {
                        ...
                        fprintf(USER_STREAM, "Static aligh succeed. Next, you may want to run: bash client.sh -- stage eight_route\n");
                }
        } 
        else 
        {
...
int Client::EightRouteStage() 
{
        ...
        if ("start" == cmd) 
        {
                ...
                else 
                {
                        ...
                        fprintf(USER_STREAM, "Eight route succeed. Next, you may want to run: bash client.sh --stage data_collect\n");
                }
        } 
        else 
        {
...
int Client::DataCollectStage() 
{
        ...
        if (cmd == "start") 
        {
                if (lines.empty()) 
                {
                        ...
                        fprintf(USER_STREAM, "Start success. At the end of the collection, you should run: bash client.sh data_collect stop\n");
                } 
                else 
                {
                        ...
                        if (s[1] == "start") 
                        {
                                AINFO << "This progress has been already started, this command will be ignored";
                                fprintf(USER_STREAM, "This progress has been already started, this command will be ignored\n");
                        } 
                        else 
                        {
                                time_file_handler << now << " start\n";
                                AINFO << "write [" << now << " start] to file " << data_collect_time_flag_file_;
                                fprintf(USER_STREAM, "Start success. At the end of the collection, you should run: bash client.sh --stage data_collect --cmd stop\n");
                        }
                }
        } 
        else if (cmd == "stop") 
        {
                ...
                else 
                {
                        ...
                        if (s[1] == "start") 
                        {
                                ...
                                fprintf(USER_STREAM, "Stop success. Next you may want to run: bash client.sh loops_check start\n");
                        } 
                        else 
                        {
                                AINFO << "This progress has been already stopped, this command will be ignored";
                                fprintf(USER_STREAM, "This progress has been already stopped, this command will be ignored\n");
                        }
                }
        } 
        else 
        {
                AINFO << "Error command, expected command are [start|stop], current command is [" << cmd << "]";
                fprintf(USER_STREAM, "Error command, expected command are [start|stop], current command is [%s]\n", cmd.c_str());
        }
        return 0;
}
...
int Client::LoopsCheckStage() 
{
        ...
        if (reached) 
        {
                ...
                fprintf(USER_STREAM, "Loops meet requirements. Next you may want to run: bash client.sh --stage eight_route\n");
        } 
        else 
        {
