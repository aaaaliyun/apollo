...
public:
        ...
        int LaneProjection(const apollo::common::math::Vec2d &vec2d, const std::string &lane_id, double *s, double *l, double *heading) const 
        {
                ...
                QUIT_IF(!ret, -3, ERROR, "lane[%s] get projection for point[%f, %f] failed", lane_id.c_str(), vec2d.x(), vec2d.y());
                *heading = lane->Heading(*s);
                return 0;
        }
...
int main(int argc, char *argv[]) 
{
        ...
        if (FLAGS_lane_to_lane) 
        {
                ...
                int ret = map_util.LaneProjection({point.x(), point.y()}, FLAGS_to_lane, &target_s, &target_l, &target_heading);
                if (ret != 0) 
                {
                        printf("lane_projection for lane[%s], s[%f] to lane_id[%s] failed\n", FLAGS_from_lane.c_str(), FLAGS_s, FLAGS_to_lane.c_str());
                        return -1;
                }
                printf("lane[%s] s[%f], l[%f], heading[%f]\n", FLAGS_to_lane.c_str(), target_s, target_l, target_heading);
