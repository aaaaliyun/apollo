...
double PncMap::LookForwardDistance(double velocity) 
{
        ...
        return forward_distance > FLAGS_look_forward_short_distance ? FLAGS_look_forward_long_distance : FLAGS_look_forward_short_distance;
}
...
bool PncMap::UpdateVehicleState(const VehicleState &vehicle_state) 
{
        ...
        if (!adc_state_.has_x() || (common::util::DistanceXY(adc_state_, vehicle_state) > FLAGS_replan_lateral_distance_threshold + FLAGS_replan_longitudinal_distance_threshold)) 
        {
                                       ...
                                       const LaneWaypoint &waypoint) const 
{
        ...
        while (i < static_cast<int>(route_indices_.size()) && !RouteSegments::WithinLaneSegment(route_indices_[i].segment, waypoint)) 
        {
                ++i;
        }
        return i;
                               ...
                               RouteSegments *segments) const 
{
        ...
        for (const auto &lane : passage.segment()) 
        {
                ...
                segments->emplace_back(lane_ptr, std::max(0.0, lane.start_s()), std::min(lane_ptr->total_length(), lane.end_s()));
        }
                              ...
                              std::list<RouteSegments> *const route_segments) 
{
        ...
        for (const int index : drive_passages) 
        {
                ...
                const PointENU nearest_point = index == passage_index ? adc_waypoint_.lane->GetSmoothPoint(adc_waypoint_.s) : PointFactory::ToPointENU(adc_state_);
                common::SLPoint sl;
                                        ...
                                        LaneWaypoint *waypoint) const 
{
        ...
        const int status = hdmap_->GetLanesWithHeading(point, kMaxDistance, state.heading(), M_PI / 2.0 + kHeadingBuffer, &lanes);
