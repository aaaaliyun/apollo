...
Path::Path(const std::vector<MapPathPoint>& path_points) : path_points_(path_points) 
{
        Init();
}

Path::Path(std::vector<MapPathPoint>&& path_points) : path_points_(std::move(path_points)) 
{
        Init();
}

Path::Path(const std::vector<MapPathPoint>& path_points, const std::vector<LaneSegment>& lane_segments) : path_points_(path_points), lane_segments_(lane_segments) 
{
        Init();
}

Path::Path(std::vector<MapPathPoint>&& path_points, std::vector<LaneSegment>&& lane_segments) : path_points_(std::move(path_points)), lane_segments_(std::move(lane_segments)) 
{
        Init();
}
...
Path::Path(const std::vector<MapPathPoint>& path_points,
           ...
           const double max_approximation_error) : path_points_(path_points), lane_segments_(lane_segments) 
{
        Init();
...
Path::Path(std::vector<MapPathPoint>&& path_points,
           ...
           const double max_approximation_error) : path_points_(std::move(path_points)), lane_segments_(std::move(lane_segments)) 
{
        Init();
        if (max_approximation_error > 0.0) 
        {
...
void Path::GetAllOverlaps(GetOverlapFromLaneFunc GetOverlaps_from_lane, std::vector<PathOverlap>* const overlaps) const 
{
        if (overlaps == nullptr) 
        {
                return;
        ...
        for (auto& overlaps_one_object : overlaps_by_id) 
        {
                ...
                {
                        if (!overlaps->empty() && overlaps->back().object_id == object_id && segment.first - overlaps->back().end_s <= kMinOverlapDistanceGap) 
                        {
...
{
        auto next = std::upper_bound(lane_overlaps_.begin(), lane_overlaps_.end(), s, [](double s, const PathOverlap& o) { return s < o.start_s; });
...
InterpolatedIndex Path::GetIndexFromS(double s) const 
{
        ...
        int high = (next_sample_id < num_sample_points_ ? std::min(num_points_, last_point_index_[next_sample_id] + 1) : num_points_);
        while (low + 1 < high) 
        {
...
std::vector<hdmap::LaneSegment> Path::GetLaneSegments(const double start_s, const double end_s) const 
{
        ...
        lanes.emplace_back(lane_segments_[start_index.id].lane, start_index.offset, lane_segments_[start_index.id].Length());
...
void PathApproximation::InitDilute(const Path& path) 
{
        ...
        while (last_idx < num_original_points - 1) 
        {
                ...
                for (; delta > 0; delta /= 2) 
                {
                        if (next_idx + delta < num_original_points && is_within_max_error(path, last_idx, next_idx + delta)) 
                        {
                                next_idx += delta;
                        }
                }
...
void PathApproximation::InitProjections(const Path& path) 
{
        ...
        for (size_t i = 0; i < projections_.size(); ++i) 
        {
                ...
                if (i + 1 < projections_.size()) 
                {
                        ...
                        for (int idx = original_ids_[i] + 1; idx < original_ids_[i + 1]; ++idx) 
                        {
                                ...
                                original_projections_.push_back(projections_[i] + std::max(0.0, std::min(proj, segment.length())));
                        }
                }
        }
        ...
        for (int i = 0; i + 1 < num_original_points; ++i) 
        {
                CHECK_LE(min_original_projections_to_right_[i], min_original_projections_to_right_[i + 1] + kMathEpsilon);
        }
        ...
        {
                while (last_index + 1 < num_original_points && max_original_projections_to_left_[last_index + 1] < proj) 
                {
                        ++last_index;
                }
                                      ...
                                      double* min_distance) const 
{
        ...
        double min_distance_sqr_with_error = Sqr(sqrt(min_distance_sqr) + max_error_per_segment_[estimate_nearest_segment_idx] + max_error_);
        *min_distance = std::numeric_limits<double>::infinity();
        ...
        for (size_t i = 0; i < segments_.size(); ++i) 
        {
                ...
                if (first_segment_idx < last_segment_idx) 
                {
                        ...
                        if (min_projection > 0.0) 
                        {
                                ...
                                {
                                        first_segment_idx = std::max(first_segment_idx, sampled_max_original_projections_to_left_[sample_index]);
                                        if (first_segment_idx >= last_segment_idx) 
                                        {
                                                first_segment_idx = last_segment_idx;
                                        } 
        ...
        if (nearest_segment_idx >= 0) 
        {
                ...
                *accumulate_s = original_accumulated_s[nearest_segment_idx] + proj;
                if ((nearest_segment_idx == 0 && proj < 0.0) || (nearest_segment_idx + 1 == num_original_segments && proj > segment.length())) 
                {
                        *lateral = prod;
                } 
                else 
