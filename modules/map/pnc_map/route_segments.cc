...
bool RouteSegments::GetWaypoint(const double s, LaneWaypoint *waypoint) const 
{
        ...
        {
                if (accumulated_s - kSegmentationEpsilon < s && s < accumulated_s + iter->end_s - iter->start_s + kSegmentationEpsilon) 
                {
                                  ...
                                  LaneWaypoint *waypoint) const 
{
        ...
        for (auto iter = begin(); iter != end(); accumulated_s += (iter->end_s - iter->start_s), ++iter) 
        {
                ...
                {
                        AERROR << "Failed to get projection from point " << point.DebugString() << " on lane " << iter->lane->id().id();
                        return false;
                }
                if (lane_s < iter->start_s - kSegmentationEpsilon || lane_s > iter->end_s + kSegmentationEpsilon) 
                {
                        continue;
                }
