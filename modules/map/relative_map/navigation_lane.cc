    ...
    google::protobuf::Map<std::string, NavigationPath> *const navigation_path) 
{
        ...
        if (FLAGS_relative_map_generate_left_boundray) 
        {
                ...
                left_boundary_type->add_types(perception_obstacles.lane_marker().left_lane_marker().lane_type());
        ...
        right_boundary_type->add_types(perception_obstacles.lane_marker().right_lane_marker().lane_type());
        ...
        for (const auto &path_point : path.path_point()) 
        {
                ...
                if (FLAGS_relative_map_generate_left_boundray) 
                {
                        ...
                        left_segment->add_point()->CopyFrom(*point + lane_left_width * Vec2d::CreateUnitVec2d(path_point.theta() + M_PI_2));
                }

                auto *right_sample = lane->add_right_sample();
                right_sample->set_s(path_point.s());
                right_sample->set_width(lane_right_width);
                right_segment->add_point()->CopyFrom(*point + lane_right_width * Vec2d::CreateUnitVec2d(path_point.theta() - M_PI_2));
        }
        return true;
}
}  // namespace

NavigationLane::NavigationLane(const NavigationLaneConfig &config) : config_(config) {}
...
bool NavigationLane::GeneratePath() 
{
        ...
        auto generate_path_on_perception_func = [this, &lane_marker]() 
        {
                ...
                double right_width = perceived_right_width_ > 0.0 ? perceived_right_width_ : default_right_width_;
        ...
        if (config_.lane_source() == NavigationLaneConfig::OFFLINE_GENERATED && navigation_line_num > 0) 
        {
                ...
                for (int i = 0; i < navigation_line_num; ++i) 
                {
                        ...
                        if (ConvertNavigationLineToPath(i, path)) 
                        {
                                ...
                                navigation_path_list_.emplace_back(i, default_left_width_, default_right_width_, current_navi_path);
                        }
                }
                ...
                double left_width = perceived_left_width_ > 0.0 ? perceived_left_width_ : default_left_width_;
                double right_width = perceived_right_width_ > 0.0 ? perceived_right_width_ : default_right_width_;
                if (!IsFloatEqual(left_width, default_left_width_) && !IsFloatEqual(right_width, default_right_width_)) 
                {
                ...
                for (auto iter = navigation_path_list_.begin(); iter != navigation_path_list_.end(); ++iter) 
                {
                        ...
                        if (prev_iter != navigation_path_list_.end()) 
                        {
                                ...
                                average_point_size = std::min(average_point_size, std::min(curr_path.path_point_size(), prev_path.path_point_size()));
                        ...
                        if (next_iter != navigation_path_list_.end()) 
                        {
                                ...
                                average_point_size = std::min(average_point_size, std::min(curr_path.path_point_size(), next_path.path_point_size()));
                                double lateral_distance_sum = 0.0;
                                for (int i = 0; i < average_point_size; ++i) 
                                {
...
void NavigationLane::MergeNavigationLineAndLaneMarker(const int line_index, common::Path *const path) 
{
        ...
        for (int i = 0; i < path->path_point_size(); ++i) 
        {
                ...
                double s = point->s();
                auto lane_maker_point = GetPathPointByS(lane_marker_path, lane_marker_index, s, &lane_marker_index);
                ...
                {
                        point->set_y(navigation_line_weight * point->y() + (1 - navigation_line_weight) * lane_maker_point.y());
                        lane_marker_index = 0;
                        continue;
                }
                *point = common::util::GetWeightedAverageOfTwoPathPoints(*point, lane_maker_point, navigation_line_weight, 1 - navigation_line_weight);
        }
}
                                                  ...
                                                  int *const matched_index) 
{
        ...
        *matched_index = i;
        ...
        const double r = (s - path.path_point(i).s()) / (path.path_point(i + 1).s() - path.path_point(i).s());
        auto p = common::util::GetWeightedAverageOfTwoPathPoints(path.path_point(i), path.path_point(i + 1), 1 - r, r);
        return p;
}
...
bool NavigationLane::ConvertNavigationLineToPath(const int line_index, common::Path *const path) 
{
        ...
        auto enu_to_flu_func = [this, dx, dy](const double enu_x, const double enu_y, const double enu_theta, double *flu_x, double *flu_y, double *flu_theta) 
        {
                if (flu_x != nullptr && flu_y != nullptr) 
                {
                        Eigen::Vector2d flu_coordinate = common::math::RotateVector2d({enu_x + dx, enu_y + dy}, -original_pose_.heading());
                ...
                {
                        *flu_theta = common::math::NormalizeAngle(common::math::NormalizeAngle(enu_theta) - original_pose_.heading());
                }
        };

        auto gen_navi_path_loop_func = [this, &navigation_path, &enu_to_flu_func](const int start, const int end, const double ref_s_base, const double max_length, common::Path *path) 
        {
                CHECK_NOTNULL(path);
        ...
        double dist = navigation_path.path_point().rbegin()->s() - navigation_path.path_point(current_project_index).s();
        ...
        if (FLAGS_enable_cyclic_rerouting && dist < config_.max_len_from_navigation_line()) 
        {
                ...
                if (item_iter != stitch_index_map_.end()) 
                {
                        ...
                        double length = navigation_path.path_point(stitch_start_index).s() - navigation_path.path_point(current_project_index).s();
                        gen_navi_path_loop_func(std::max(0, current_project_index - 3), stitch_start_index + 1, 0.0, length, path);
                        if (length > config_.max_len_from_navigation_line()) 
                        {
                                return true;
                        }
                        gen_navi_path_loop_func(stitch_end_index, navigation_path.path_point_size(), length, config_.max_len_from_navigation_line(), path);
                        return true;
                }
        }
        ...
        gen_navi_path_loop_func(std::max(0, current_project_index - 3), navigation_path.path_point_size(), 0.0, config_.max_len_from_navigation_line(), path);
        return true;
}
                                                    ...
                                                    const int line_index) 
{
        ...
        auto check_distance_func = [this, &path, &path_size](const int project_index, double *project_distance) 
        {
                ...
                if (std::fabs(flu_x) < config_.max_distance_to_navigation_line() / 2.0 && std::fabs(flu_y) < config_.max_distance_to_navigation_line() * 2.0) 
                {
                        return true;
                }
                return false;
...
void NavigationLane::ConvertLaneMarkerToPath(const perception::LaneMarkers &lane_marker, common::Path *const path) 
{
        CHECK_NOTNULL(path);
        ...
        double path_c1 = (left_lane.c1_heading_angle() * left_quality + right_lane.c1_heading_angle() * right_quality) / quality_divider;

        double path_c2 = (left_lane.c2_curvature() * left_quality + right_lane.c2_curvature() * right_quality) / quality_divider;
        ...
        for (double z = start_s; z <= path_range; z += unit_z) 
        {
                double x1 = z;
                double y1 = 0;
                if (left_lane.view_range() > config_.min_view_range_to_use_lane_marker() || right_lane.view_range() > config_.min_view_range_to_use_lane_marker()) 
                {
        ...
        if (perceived_lane_width < 2.0 * config_.min_lane_half_width() || perceived_lane_width > 2.0 * config_.max_lane_half_width()) 
        {
                               ...
                               MapMsg *const map_msg) const 
{
        ...
        if (navigation_path_list_.empty()) 
        {
                if (std::get<3>(current_navi_path_tuple_) != nullptr) 
                {
                        ...
                        return CreateSingleLaneMap(current_navi_path_tuple_, map_config, perception_obstacles_, hdmap, navigation_path);
                } 
                else 
                {
                        return false;
...
void NavigationLane::UpdateStitchIndexInfo() 
{
        ...
        for (int i = 0; i < navigation_line_num; ++i) 
        {
                ...
                if (!navigation_info_.navigation_path(i).has_path() || navigation_path.path_point_size() < kMinPathPointSize) 
                {
                        continue;
                }
                ...
                for (int m = 0; m < path_size; ++m) 
                {
                        ...
                        for (int n = path_size - 1; n >= 0; --n) 
                        {
                                ...
                                double current_distance = DistanceXY(navigation_path.path_point(m), navigation_path.path_point(n));
