...
bool IsAllowedToCross(const LaneBoundary& boundary) 
{
        ...
        {
                if (boundary_type.types(0) != LaneBoundaryType::DOTTED_YELLOW && boundary_type.types(0) != LaneBoundaryType::DOTTED_WHITE) 
                {
                        return false;
                }
        }
...
bool GraphCreator::Create() 
{
        ...
        if (!absl::EndsWith(dump_topo_file_path_, ".bin") && !absl::EndsWith(dump_topo_file_path_, ".txt")) 
        {
...
bool GraphCreator::IsValidUTurn(const hdmap::Lane& lane, const double radius) 
{
        ...
        if (lane.central_curve().segment().empty() || !lane.central_curve().segment(0).has_line_segment()) 
        {
                return false;
        }
        ...
        {
                points.insert(points.end(), segment.line_segment().point().begin(), segment.line_segment().point().end());
        }
        if (points.empty()) 
        {
                return false;
        ...
        if (std::fabs(s2) < kMathEpsilon) 
        {  
                // q4 is the circle center
                return q4.DistanceTo(p1) >= radius;
        }
        if (std::fabs(s1 - s2) < kMathEpsilon) 
        {  
                // parallel case, a wide u-turn
                return true;
        }
