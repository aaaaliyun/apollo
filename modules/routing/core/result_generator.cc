...
bool ResultGenerator::IsReachableToWithChangeLane(const TopoNode* to_node, const PassageInfo& from_nodes, NodeWithRange* reachable_node) 
{
...
void ResultGenerator::ExtendBackward(const TopoRangeManager& range_manager, const PassageInfo& prev_passage, PassageInfo* const curr_passage) 
{
        ...
        if (!IsCloseEnough(front_node.StartS(), 0.0)) 
        {
                ...
                {
                        double temp_s = prev_passage.nodes.front().StartS() / prev_passage.nodes.front().FullLength() * front_node.FullLength();
                                    ...
                                    PassageInfo* const curr_passage) 
{
        ...
        if (!IsCloseEnough(back_node.EndS(), back_node.FullLength())) 
        {
                if (!range_manager.Find(back_node.GetTopoNode())) 
                {
                        ...
                        {
                                double temp_s = next_passage.nodes.back().EndS() / next_passage.nodes.back().FullLength() * back_node.FullLength();
                ...
                for (const auto& edge : curr_passage->nodes.back().GetTopoNode()->OutToSucEdge()) 
                {
                        ...
                        if (IsReachableFromWithChangeLane(succ_node, next_passage, &reachable_node)) 
                        {
                                ...
                                else 
                                {
                                        ...
                                        {
                                                double push_end_s = reachable_node.EndS() / reachable_node.FullLength() * succ_node->Length();
...
void LaneNodesToPassageRegion(const std::vector<NodeWithRange>::const_iterator first, const std::vector<NodeWithRange>::const_iterator last, Passage* const passage) 
{
...
bool ResultGenerator::GeneratePassageRegion(const std::string& map_version, const RoutingRequest& request, const std::vector<NodeWithRange>& nodes, const TopoRangeManager& range_manager, RoutingResponse* const result) 
{
...
bool ResultGenerator::GeneratePassageRegion(const std::vector<NodeWithRange>& nodes, const TopoRangeManager& range_manager, RoutingResponse* const result) 
{
...
void ResultGenerator::AddRoadSegment(const std::vector<PassageInfo>& passages, const std::pair<std::size_t, std::size_t>& start, const std::pair<std::size_t, std::size_t>& end, RoutingResponse* result) 
{
        ...
        for (std::size_t i = start.first; i <= end.first && i < passages.size(); ++i) 
        {
                ...
                auto end_iter = passages[i].nodes.cbegin() + (i == end.first ? end.second : passages[i].nodes.size());
