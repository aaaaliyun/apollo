...
public:
        ...
        explicit BridgeProtoDiserializedBuf(const std::string &topic_name) : topic_name_(topic_name) {}
...
{
        if (strcmp(proto_name_.c_str(), header.GetMsgName().c_str()) == 0 && sequence_num_ == header.GetMsgID()) 
        {
                return true;
        }
        return false;
...
bool BridgeProtoDiserializedBuf<T>::Initialize(const BridgeHeader &header) 
{
        ...
        int status_size = static_cast<int>(total_frames_ / INT_BITS + ((total_frames_ % INT_BITS) ? 1 : 0));
...
template <typename T>
bool BridgeProtoDiserializedBuf<T>::Initialize(const BridgeHeader &header, std::shared_ptr<cyber::Node> node) 
{
