...
bool UDPBridgeMultiReceiverComponent::MsgHandle(int fd) 
{
        ...
        bytes = static_cast<int>(recvfrom(fd, total_buf, total_recv, 0, (struct sockaddr *)&client_addr, &sock_len));
...
bool UDPBridgeMultiReceiverComponent::RemoveInvalidBuf(uint32_t msg_id, const std::string &msg_name) 
{
        if (msg_id == 0) 
        {
                return false;
        ...
        for (; itor != proto_list_.end();) 
        {
                if ((*itor)->GetMsgID() < msg_id && strcmp((*itor)->GetMsgName().c_str(), msg_name.c_str()) == 0) 
                {
                        itor = proto_list_.erase(itor);
                        continue;
                }
