...
{
        const auto junctions = PredictionMap::GetPNCJunctions({point.x(), point.y()}, FLAGS_junction_search_radius);
        if (junctions.empty() || junctions.front() == nullptr) 
        {
                return false;
        }
        const auto& junction_info = junctions.front();
        if (junction_info != nullptr && junction_info->pnc_junction().polygon().point_size() >= 3) 
        {
...
{
        const auto junctions = PredictionMap::GetJunctions({point.x(), point.y()}, FLAGS_junction_search_radius);
        if (junctions.empty() || junctions.front() == nullptr) 
        {
                return false;
        }
        const auto& junction_info = junctions.front();
        if (junction_info != nullptr && junction_info->junction().polygon().point_size() >= 3) 
        {
                *junction_id = junction_info->id().id();
                return true;
        }
...
{
        auto* manager = FrameManager::Instance();
        static auto planning_reader = manager->CreateOrGetReader<ADCTrajectory>(FLAGS_planning_trajectory_topic);
